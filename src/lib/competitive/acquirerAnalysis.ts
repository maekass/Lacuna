...
export interface Acquirer {
  id: string;
  name: string;
  type: 'strategic_healthcare' | 'strategic_tech' | 'private_equity' | 'venture_capital' | 'corporate_health';
...
export interface AcquiredCompany {
  ...
  name: string;
  sector: string;
  stage: 'pre_seed' | 'seed' | 'series_a' | 'series_b' | 'series_c' | 'series_d_plus' | 'public';
  yearFounded: number;
  yearAcquired: number;
...
export interface PortfolioComposition {
  ...
  // Geography
  geographyComposition: { geography: string; count: number; percentage: number }[];
...
export function analyzePortfolio(
  ...
  acquiredCompanies.forEach(c => sectorCounts.set(c.sector, (sectorCounts.get(c.sector) || 0) + 1));
  ...
  acquiredCompanies.forEach(c => stageCounts.set(c.stage, (stageCounts.get(c.stage) || 0) + 1));
  ...
  acquiredCompanies.forEach(c => geoCounts.set(c.geography, (geoCounts.get(c.geography) || 0) + 1));
  ...
  const valueStats = {
    ...
    mean: values.length > 0 ? values.reduce((s, v) => s + v, 0) / values.length : 0,
  ...
  const ages = acquiredCompanies.map(c => c.yearAcquired - c.yearFounded).filter(a => a >= 0);
  ...
  const descriptivePattern = `Acquired ${acquiredCompanies.length} companies. ` +
    `Most common sector: ${topSector.sector} (${topSector.count} deals, ${topSector.percentage.toFixed(0)}%). ` +
    `Most common stage: ${topStage.stage.replace(/_/g, ' ')} (${topStage.count} deals). ` +
...
export interface ExternalEvent {
  year: number;
  type: 'ipo' | 'earnings' | 'ceo_change' | 'strategy_announcement' | 'regulatory';
  description: string;
}
...
export interface VelocityAnalysis {
  acquirerId: string;
  acquirerName: string;
  yearlyData: { year: number; count: number; cumulative: number; value: number }[];
...
export function analyzeVelocity(
  ...
  for (let y = minYear; y <= maxYear; y++) {
    ...
    yearlyData.push({ year: y, count: data.count, cumulative, value: data.value });
  }
  ...
  // Descriptive pattern
  let descriptivePattern = `Made ${acqRecords.length} acquisitions between ${minYear} and ${maxYear}. `;
  if (periods.length === 2) {
    const change = periods[1].avgPerYear - periods[0].avgPerYear;
    descriptivePattern += `Velocity changed from ${periods[0].avgPerYear.toFixed(1)}/year in early period to ${periods[1].avgPerYear.toFixed(1)}/year in late period (${change > 0 ? '+' : ''}${change.toFixed(1)} difference). `;
  }
  descriptivePattern += `Causality unclear: trends may reflect market conditions, not strategic shift.`;
...
export interface ContestableTarget {
  ...
  stage: string;
  yearFounded: number;
  potentialBuyers: { acquirerId: string; acquirerName: string; reason: string }[];
  contestability: 'high' | 'medium' | 'low';
}

export interface MarketStructureResult {
  contestableTargets: ContestableTarget[];
  sectorContestability: { sector: string; contestable: number; total: number; rate: number }[];
  caveat: string;
  descriptivePattern: string;
}
...
export function analyzeMarketStructure(
      ...
      if (acquirerSectors.has(company.sector)) {
        ...
        if (age <= 8) { // Reasonable age for acquisition
          potentialBuyers.push({
            acquirerId: acquirer.id,
            acquirerName: acquirer.name,
            reason: `Previously acquired in ${company.sector}, company age ${age}y compatible`
          });
        }
      }
    });
  ...
  contestableTargets.forEach(t => {
    if (!sectorMap.has(t.sector)) sectorMap.set(t.sector, { contestable: 0, total: 0 });
  ...
  const highContestableCount = contestableTargets.filter(t => t.contestability === 'high').length;
  const totalCompanies = companies.length;
  
  const descriptivePattern = `Of ${totalCompanies} companies, ${highContestableCount} have 3+ plausible buyers (high contestability). ` +
    `Most contestable sector: ${sectorContestability[0]?.sector || 'N/A'} (${(sectorContestability[0]?.rate * 100 || 0).toFixed(0)}% rate). ` +
    `Least contestable: ${sectorContestability[sectorContestability.length - 1]?.sector || 'N/A'}.`;
  
  return {
    contestableTargets,
    sectorContestability,
    caveat: 'Assumes all buyers have equal interest in all companies. Reality: buyers have internal preferences (unobserved).',
    descriptivePattern
  };
}
...
export function compareAcquirerTypes(
  ...
  typeGroups.forEach((typeAcquirers, type) => {
    ...
    const typeAcqs = acquisitions.filter(a => typeAcquirerIds.has(a.acquirerId));
    ...
    // Stage distribution
    const stageCounts = new Map<string, number>();
    typeCompanies.forEach(c => stageCounts.set(c.stage, (stageCounts.get(c.stage) || 0) + 1));
    ...
    // Sector distribution
    const sectorCounts = new Map<string, number>();
    typeCompanies.forEach(c => sectorCounts.set(c.sector, (sectorCounts.get(c.sector) || 0) + 1));
    ...
    // Value stats
    const values = typeCompanies.map(c => c.acquisitionValue).filter((v): v is number => v !== undefined && v > 0);
    const ages = typeCompanies.map(c => c.yearAcquired - c.yearFounded).filter(a => a >= 0);
    ...
    byType.push({
      ...
      sectorDistribution,
      valueStats: {
        medianTargetValue: values.length > 0 ? [...values].sort((a, b) => a - b)[Math.floor(values.length / 2)] : 0,
        avgTargetAge: ages.length > 0 ? ages.reduce((s, v) => s + v, 0) / ages.length : 0,
        medianTargetAge: ages.length > 0 ? [...ages].sort((a, b) => a - b)[Math.floor(ages.length / 2)] : 0
      }
    });
  });
  ...
  if (strategic && pe) {
    const stratLateStage = strategic.stageDistribution
      .filter(s => ['series_b', 'series_c', 'series_d_plus', 'public'].includes(s.stage))
    ...
    if (stratLateStage > peLateStage + 15) {
      findings.push(`Strategic healthcare acquirers focus more on late-stage (${stratLateStage.toFixed(0)}%) than PE (${peLateStage.toFixed(0)}%)`);
    } else if (peLateStage > stratLateStage + 15) {
      findings.push(`PE acquirers focus more on late-stage (${peLateStage.toFixed(0)}%) than strategic (${stratLateStage.toFixed(0)}%)`);
    }
  }
  ...
  if (tech && health) {
    ...
    if (Math.abs(techSectorDiversity - healthSectorDiversity) >= 2) {
      const moreSpread = techSectorDiversity > healthSectorDiversity ? 'tech' : 'healthcare';
      findings.push(`${moreSpread} acquirers span more sectors (${moreSpread === 'tech' ? techSectorDiversity : healthSectorDiversity} vs ${moreSpread === 'tech' ? healthSectorDiversity : techSectorDiversity})`);
    }
  }
  ...
  byType.forEach(t => {
    if (t.valueStats.medianTargetValue > 0) {
      findings.push(`${t.type.replace(/_/g, ' ')} median target value: $${t.valueStats.medianTargetValue}M (n=${t.totalAcquisitions})`);
    }
  });
  ...
  return {
    ...
    caveat: 'Sample sizes per type are small. Comparisons descriptive only; not statistically tested.'
  };
}
