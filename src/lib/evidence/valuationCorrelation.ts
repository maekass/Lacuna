...
export function computeValuationCorrelation(
  data: CompanyEvidence[],
): CorrelationResult {
  const disclosed = data.filter((d) =>
  ...
  if (scoreSpread === 0) {
    ...
    insight = `Evidence maturity shows ${
      r > 0 ? "positive" : "negative"
    } correlation with deal value (r=${
      r.toFixed(2)
    }). Companies with scores above 50 were acquired at ${
      premiumMultiple.toFixed(1)
    }x the average of lower-scored companies.`;
  } else if (Math.abs(r) > 0.25) {
    insight = `Moderate correlation detected (r=${
      r.toFixed(2)
    }). High-evidence companies (score >= 50) averaged $${
      (avgHigh / 1000).toFixed(1)
    }B vs $${(avgLow / 1000).toFixed(1)}B for lower-scored firms — a ${
      premiumMultiple.toFixed(1)
    }x premium.`;
  } else {
    insight = `Weak linear correlation (r=${
      r.toFixed(2)
    }) — deal value may depend more on strategic fit, market timing, or competitive dynamics than clinical evidence alone. This is common in early-stage health tech where platform value matters.`;
  }
