...
export function bcaCI(
  ...
  if (n < 4) {
    return { estimate: θhat, lower: θhat, upper: θhat, level, method: "percentile", B: 0, warning: `n=${n} too small for bootstrap` };
  }
  ...
  const θstar: number[] = [];
  for (let b = 0; b < B; b++) {
    const resample = Array.from({ length: n }, () => data[Math.floor(rng() * n)]);
  ...
  if (n < 10) warnings.push(`n=${n} < 10 — jackknife acceleration unreliable; BCa degrades to percentile`);
  if (!isFinite(z0)) warnings.push("z₀ undefined (all replicates equal); returning percentile interval");
  ...
  if (p1 >= p2) { p1 = alpha / 2; p2 = 1 - alpha / 2; }
...
export function meanBCaCI(data: number[], level = 0.95, seed = 42): BootstrapCIResult {
  return bcaCI(data, (s) => s.reduce((a, b) => a + b, 0) / s.length, 1999, level, seed);
}

/** Convenience: BCa CI for the median */
export function medianBCaCI(data: number[], level = 0.95, seed = 42): BootstrapCIResult {
  return bcaCI(
    data,
    (s) => {
...
export function diffMeanBCaCI(
  ...
  const combined = [...groupA.map((v) => ({ v, g: 0 })), ...groupB.map((v) => ({ v, g: 1 }))];
