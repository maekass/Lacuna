...
export function calculateDemographicParity(
  ...
  const womenFoundedCompanies = validCompanies.filter(c =>
    c.founders.some(f => f.inferredGender === 'female' && f.confidence >= GENDER_INFERENCE_QUALITY.highConfidenceThreshold)
  );
  ...
  const menFoundedCompanies = validCompanies.filter(c => !womenFoundedCompanies.includes(c));
  ...
  const se = Math.sqrt(
    (womenFoundedAcqRate * (1 - womenFoundedAcqRate)) / Math.max(1, womenFoundedCompanies.length) +
    (menFoundedAcqRate * (1 - menFoundedAcqRate)) / Math.max(1, menFoundedCompanies.length)
  );
  ...
  const ifAllAmbiguousMenRate = acquiredWomenFounded.length / Math.max(1, acquired.length);
  
  const limitations = [
    `Sample size n=${n} provides only ${(observedPower * 100).toFixed(0)}% statistical power to detect a ${(parityDifference * 100).toFixed(0)}pp difference`,
    `Gender inference has ~${(GENDER_INFERENCE_QUALITY.errorRate * 100).toFixed(0)}% error rate; ${ambiguousFounders.length} founders flagged as ambiguous`,
  ...
  return {
    ...
    sensitivityRange: {
      ...
      interpretation: `Disparity ranges from ${(ifAllAmbiguousMenRate * 100).toFixed(0)}% to ${(ifAllAmbiguousWomenRate * 100).toFixed(0)}% depending on ambiguous name assignment`
    },
    limitations,
    recommendations
  };
...
export function analyzeFounderCharacteristics(
  ...
  const getSectorDist = (cos: CompanyWithFounders[]): Record<string, number> => {
  ...
  const avgAcqValue = (cos: CompanyWithFounders[]): number => {
    ...
    return acquired.length > 0
      ? acquired.reduce((sum, c) => sum + (c.acquisitionValue || 0), 0) / acquired.length
      : 0;
  };
  ...
  Object.keys(womenSectors).forEach(sector => {
    ...
    if (Math.abs(womenPct - menPct) > 0.15) {
      systemicDifferences.push(
        `${sector}: ${(womenPct * 100).toFixed(0)}% women vs ${(menPct * 100).toFixed(0)}% men`
      );
    }
  });
