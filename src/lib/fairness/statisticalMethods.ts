...
function calculateFishersPValue(a: number, b: number, c: number, d: number): number {
...
export function powerAnalysis(
  ...
  const recommendedSampleSize = observedDifference > 0
    ? Math.ceil(2 * Math.pow((zAlpha + zBeta80) / observedDifference, 2) * p1 * (1 - p1))
    : Infinity;
...
export function logisticRegression(
  ...
  for (iterations = 0; iterations < maxIterations; iterations++) {
    ...
    const hessian: number[][] = Array(pi).fill(null).map(() => new Array(pi).fill(0));
  ...
  const informationMatrix: number[][] = Array(pi).fill(null).map(() => new Array(pi).fill(0));
  ...
  const standardErrors = covMatrix ? covMatrix.map((row, i) => Math.sqrt(Math.abs(row[i]))) : new Array(pi).fill(0);
...
export { normalCDF, inverseNormalCDF };
