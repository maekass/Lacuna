...
export function validateVerifiedDataset(dataset: VerifiedDataset): ValidationReport {
  ...
  if (!dataset.provenance.disclaimer?.trim()) {
    push(errors, { code: 'provenance.disclaimer', severity: 'error', message: 'provenance.disclaimer is required' });
  }
  ...
  for (const d of dataset.acquisitions) {
    if (dealIds.has(d.id)) {
      push(errors, { code: 'deal.duplicateId', severity: 'error', message: `Duplicate deal id "${d.id}"`, entity: d.id });
    }
    dealIds.add(d.id);
    ...
    if (!d.source?.trim()) {
      push(errors, { code: 'deal.source', severity: 'error', message: `Deal "${d.id}" missing source`, entity: d.id });
    }
