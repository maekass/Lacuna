...
export function alertApiFailure(url: string, status: number, detail?: string): void {
  emit({
    type: 'api_failure',
    level: 'error',
...
export function alertNewDeal(deal: {
  ...
  emit({
    type: 'new_deal',
    level: 'info',
    message: `New women's health candidate (${deal.confidence}): ${deal.targetName ?? 'unknown target'}`,
    timestamp: new Date().toISOString(),
    context: deal,
  });
}
