...
export function formatLacunaForGamma(
  ...
  sections.push('');
  sections.push('Source: Lacuna — verified public filings (SEC EDGAR, press releases, ClinicalTrials.gov)');
  sections.push(`Dataset: ${dataset.acquisitions.length} acquisitions · ${dataset.companies.length} companies · ${dataset.acquirers.length} acquirers`);
  sections.push('');
  ...
  sections.push('');
  sections.push('---');
  sections.push('Data provenance: All M&A data sourced from public SEC EDGAR filings, company press releases, and verified news coverage. Licensed under BUSL 1.1. Educational use only — not investment advice.');

  return sections.join('\n');
}

function formatDealOverview(dataset: LacunaDataset): string[] {
  const totalValue = dataset.acquisitions.reduce((sum, a) => sum + (a.dealValue || 0), 0);
  const disclosed = dataset.acquisitions.filter((a) => a.dealValue && a.dealValue > 0).length;
  ...
  return [
    ...
    `- Deals with disclosed pricing: ${disclosed} of ${dataset.acquisitions.length} (${Math.round((disclosed / dataset.acquisitions.length) * 100)}%)`,
...
function formatTopDeals(dataset: LacunaDataset): string[] {
  ...
  for (const deal of sorted) {
    const year = deal.announcedDate ? new Date(deal.announcedDate).getFullYear() : 'N/A';
    const value = deal.dealValue ? `$${(deal.dealValue / 1000).toFixed(1)}B` : 'Undisclosed';
    lines.push(`- ${deal.acquirerName} → ${deal.targetName} (${year}): ${value}`);
  }
...
function formatAcquirerAnalysis(dataset: LacunaDataset): string[] {
  ...
  const sorted = Object.entries(acquirerDeals).sort((a, b) => b[1].count - a[1].count).slice(0, 10);
  const lines = ['## Most Active Acquirers', ''];

  for (const [name, data] of sorted) {
    const valueStr = data.value > 0 ? ` · $${(data.value / 1000).toFixed(1)}B total` : '';
