 ...
 */
export function calculateOsterDelta(config: SensitivityConfig): OsterDeltaResult {
  const {
  ...
  if (rSquaredMovement <= 0) {
    return {
      delta: Infinity,
      interpretation: 'No coefficient movement detected - model may be misspecified',
      robustnessThreshold: 1.0,
      isRobust: false,
      criticalDelta: 1.0,
      explanation: 'R² did not increase when adding confounders. Check model specification.'
    };
  }
  ...
  const delta = (coefficientMovement ** 2) / (rSquaredMovement * outcomeVariance);
  ...
  if (delta > 3) {
    ...
    explanation = `An unobserved confounder would need to be ${delta.toFixed(2)}x stronger than all observed confounders combined to explain away the treatment effect. This is implausibly large.`;
  } else if (delta > 1.5) {
    interpretation = 'Moderately robust to unobserved confounding';
    explanation = `An unobserved confounder would need to be ${delta.toFixed(2)}x stronger than observed confounders to nullify the result. This is possible but unlikely.`;
  } else if (delta > 0.5) {
    interpretation = 'Potentially fragile - further investigation needed';
    explanation = `An unobserved confounder only ${delta.toFixed(2)}x as strong as observed confounders could flip the result. Caution warranted.`;
  } else {
    interpretation = 'Fragile to unobserved confounding';
    explanation = `A relatively weak unobserved confounder (${delta.toFixed(2)}x observed strength) could explain away the result. Do not rely on this causal claim.`;
  }
...
export function calculateRotnitzkyBounds(
  ...
  if (lowerBound > 0 || upperBound < 0) {
    interpretation = `Treatment effect remains ${lowerBound > 0 ? 'positive' : 'negative'} even under maximum confounding (${maxConfoundingStrength * 100}% strength). Result is robust.`;
  } else if (lowerBound < 0 && upperBound > 0 && ate > 0) {
    interpretation = `Treatment effect could flip to negative under strong confounding. Point estimate is positive but bounds include zero and negative values.`;
  } else if (lowerBound < 0 && upperBound > 0 && ate < 0) {
    interpretation = `Treatment effect could flip to positive under strong confounding. Point estimate is negative but bounds include zero and positive values.`;
  } else {
    interpretation = 'Bounds are wide - high uncertainty about true treatment effect.';
  }
...
- Treatment Effect (with confounders): ${config.treatmentEffectWithConfounders.toFixed(3)}
...
- Robustness: ${oster.isRobust ? 'PASS' : 'FAIL'} (threshold: ${oster.robustnessThreshold})
  `.trim();

  // Generate transparency statement
  const transparencyStatement = generateTransparencyStatement(config, oster, rotnitzky);
...
function generateTransparencyStatement(
  ...
  const assumptions = [
    ...
    `- Selection bias exceeding ${(rotnitzky.biasRange[1] / Math.abs(rotnitzky.pointEstimate) * 100).toFixed(1)}% of the treatment effect`,
    `- Systematic measurement error in treatment assignment exceeding ${(0.1 * 100).toFixed(0)}%`,
    ``,
    `BOTTOM LINE: ${oster.isRobust 
...
export function scenarioBounds(
  ...
  se: number,
  r2: number
): Array<{ scenario: string; maxConfounding: number; bounds: RotnitzkyBounds }> {
  const scenarios = [
...
export function generateVisualizationData(
  ...
  const zone: 'robust' | 'moderate' | 'fragile' = 
    oster.delta > 2 ? 'robust' : oster.delta > 1 ? 'moderate' : 'fragile';
