...
export function bayesianEstimate(
  ...
  return {
    ...
    credibleInterval: [round(credibleInterval[0], 4), round(credibleInterval[1], 4)],
...
export const PRE_REGISTERED_HYPOTHESES: PreRegisteredHypothesis[] = [
  {
    ...
    description: 'Fertility-focused companies command higher acquisition multiples',
  ...
  },
  {
    ...
    description: 'Late-stage companies have lower valuation growth due to maturity',
    expectedEffect: -0.2,
    direction: 'negative',
  ...
  },
  {
    ...
    description: 'Mental health companies trade at lower multiples due to reimbursement challenges',
    expectedEffect: -0.15,
    direction: 'negative',
...
export function crossFittingAnalysis(
  ...
  if (nTotal < 20) {
    return {
      ...
      isReliable: false,
      warning: `n=${nTotal} insufficient for cross-fitting. Using all data for single estimate with strong priors.`
    };
  }
...
export function smallSampleCausalAnalysis(
  ...
  const crossFit = crossFittingAnalysis(nObservations, Array.from({ length: nObservations }, (_, i) => i));
  ...
  const recommendations = [
    ...
    `Prior influence: ${(mainEffects.priorInfluence * 100).toFixed(1)}% - acknowledge prior dominates with n=${nObservations}`
  ];
...
- Cross-fitting feasibility: ${crossFit.isReliable ? 'Marginal' : 'Not feasible'}
