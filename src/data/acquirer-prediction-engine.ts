...
export const STRATEGIC_ACQUIRERS: AcquirerProfile[] = [
  {
    ...
    cashOnHand: 12,
    acquisitionHistory: [
      { targetName: 'Signify Health', targetSector: 'home_health', dealValue: 8000, dealDate: '2022-09', stageAtAcquisition: 'growth', strategicRationale: 'Expand primary care services' }
    ],
    ...
    recentActivity: 'high',
    strategicPriorities: ['primary care expansion', 'digital health', 'women health services'],
  ...
  },
  {
    ...
    cashOnHand: 25,
    acquisitionHistory: [
      { targetName: 'Change Healthcare', targetSector: 'healthcare_it', dealValue: 13000, dealDate: '2022-07', stageAtAcquisition: 'public', strategicRationale: 'Healthcare data and analytics' },
      { targetName: 'LHC Group', targetSector: 'home_health', dealValue: 5400, dealDate: '2022-03', stageAtAcquisition: 'public', strategicRationale: 'Home health expansion' }
    ],
    sectorFocus: ['healthcare_it', 'home_health', 'digital_health', 'womens_health'],
    stagePreference: ['growth', 'late_stage', 'public'],
    typicalDealSize: { min: 1000, max: 15000 },
    recentActivity: 'high',
    strategicPriorities: ['Optum platform', 'digital health', 'home-based care'],
  ...
  },
  {
    ...
    marketCap: 2.5,
    acquisitionHistory: [
      { targetName: 'Livongo', targetSector: 'digital_therapeutics', dealValue: 18500, dealDate: '2020-08', stageAtAcquisition: 'public', strategicRationale: 'Chronic care management platform' }
    ],
    ...
    strategicPriorities: ['whole person care', 'AI integration', 'chronic conditions'],
    integrationStyle: 'platform'
  },
  {
    id: 'ro-health',
    ...
    marketCap: 7,
    acquisitionHistory: [
      { targetName: 'Modern Fertility', targetSector: 'fertility', dealValue: 150, dealDate: '2021-05', stageAtAcquisition: 'series_a', strategicRationale: 'Women health expansion' }
    ],
    ...
    recentActivity: 'medium',
    strategicPriorities: ['platform expansion', 'women health', 'fertility services'],
  ...
  },
  {
    ...
    recentActivity: 'low',
    strategicPriorities: ['fertility benefits', 'employer solutions', 'platform scale'],
    integrationStyle: 'hands_on'
  },
  {
    ...
    type: 'pe',
    acquisitionHistory: [
      { targetName: 'IVF Florida', targetSector: 'fertility', dealValue: 200, dealDate: '2019-06', stageAtAcquisition: 'growth', strategicRationale: 'Clinic consolidation' }
    ],
    sectorFocus: ['fertility', 'womens_health'],
    stagePreference: ['growth', 'late_stage'],
    typicalDealSize: { min: 100, max: 500 },
    recentActivity: 'medium',
    strategicPriorities: ['clinic consolidation', 'platform building', 'geographic expansion'],
    integrationStyle: 'hands_on'
  },
  {
    id: 'warburg-pincus',
    ...
    type: 'pe',
    acquisitionHistory: [
      { targetName: 'Syneos Health', targetSector: 'cro', dealValue: 7100, dealDate: '2017-08', stageAtAcquisition: 'public', strategicRationale: 'Healthcare services platform' }
    ],
    sectorFocus: ['healthcare_services', 'digital_health', 'womens_health'],
    stagePreference: ['growth', 'late_stage'],
    typicalDealSize: { min: 500, max: 5000 },
    recentActivity: 'high',
    strategicPriorities: ['healthcare services', 'platform investing', 'digital health'],
    integrationStyle: 'hands_off'
  }
];
...
export function calculateMatchScore(
  ...
  // Generate rationale
  const keyRationale = generateRationale(company, acquirer, strategicFit, marketFit);
...
function calculateStrategicFit(company: CompanyProfile, acquirer: AcquirerProfile): number {
  let score = 50; // Base
...
function calculateCulturalFit(company: CompanyProfile, acquirer: AcquirerProfile): number {
...
function calculateFinancialFit(company: CompanyProfile, acquirer: AcquirerProfile): number {
  ...
  const rangeMid = (acquirer.typicalDealSize.min + acquirer.typicalDealSize.max) / 2;
  const distanceFromMid = Math.abs(estimatedValue - rangeMid);
  const rangeWidth = acquirer.typicalDealSize.max - acquirer.typicalDealSize.min;
  
  return Math.max(40, 100 - (distanceFromMid / rangeWidth) * 40);
}

function calculateMarketFit(company: CompanyProfile, acquirer: AcquirerProfile): number {
  let score = 50;
  
  // Sector alignment
  if (acquirer.sectorFocus.includes(company.sector)) score += 25;
  else if (acquirer.sectorFocus.some(s => areSectorsRelated(s, company.sector))) score += 15;
  
  // Stage alignment
  if (acquirer.stagePreference.includes(company.stage)) score += 25;
  else if (isStageNearPreference(company.stage, acquirer.stagePreference)) score += 15;
...
function estimateValue(
  ...
  const rationale = `Based on ${adjustedMultiple.toFixed(1)}x funding multiple (${company.sector} sector), adjusted for ${matchScore}% strategic fit`;
  
  return { min: Math.round(min), max: Math.round(max), median: Math.round(median), rationale };
}
...
function generateRationale(
  ...
  if (strategicFit >= 70) {
    rationale.push(`Strong strategic fit: ${acquirer.name}'s priorities align with ${company.name}'s capabilities`);
  }
  
  if (acquirer.sectorFocus.includes(company.sector)) {
    rationale.push(`Sector match: ${acquirer.name} actively investing in ${company.sector}`);
  }
  ...
  if (similarAcqs.length > 0) {
    rationale.push(`Prior acquisitions: ${acquirer.name} acquired ${similarAcqs.length} similar companies`);
  }
  
  if (acquirer.stagePreference.includes(company.stage)) {
    rationale.push(`Stage match: ${acquirer.name} prefers ${company.stage} companies`);
  }
  
  return rationale.length > 0 ? rationale : ['General strategic interest in healthcare sector'];
}
...
export function analyzeCompetitiveDynamics(
  ...
  const predictedWinner = topMatches[0].likelihood === 'high' ? topMatches[0].acquirer : undefined;
  const winProbability = topMatches[0].matchScore / 100;
  
  // Competitive threat assessment
  const highInterestCount = topMatches.filter(m => m.likelihood === 'high').length;
  const competitiveThreatLevel = highInterestCount >= 2 ? 'high' : highInterestCount === 1 ? 'medium' : 'low';
  
  // Estimate bidding war premium
  const estimatedBiddingWarPremium = competitiveThreatLevel === 'high' ? 25 :
    competitiveThreatLevel === 'medium' ? 15 : 0;
  
  // Fair value estimate (average of top 3)
  const top3Values = topMatches.slice(0, 3).map(m => m.estimatedValue.median);
  const fairValueMedian = top3Values.reduce((a, b) => a + b, 0) / top3Values.length;
...
function estimateTimeline(company: CompanyProfile, topMatch: AcquirerMatch): { months: number; triggers: string[] } {
  const baseMonths = company.stage === 'seed' ? 36 :
    company.stage === 'series_a' ? 24 :
    company.stage === 'series_b' ? 18 :
    12;
  
  const triggers: string[] = [];
  
  if (company.fdaStatus === 'pending') triggers.push('FDA approval/clearance');
  if (company.clinicalTrials && company.clinicalTrials > 0) triggers.push('Positive trial results');
  if (company.stage === 'series_b') triggers.push('Series C funding');
  if (topMatch.likelihood === 'high') triggers.push('Strategic acquirer approach');
...
function getSectorComparables(sector: string): ComparableDeal[] {
  ...
  const mockDeals: ComparableDeal[] = [
    { targetName: 'Modern Fertility', acquirerName: 'Ro Health', dealValue: 150, dealDate: '2021-05', sector: 'fertility', stage: 'series_a', revenueMultiple: 3.2 },
    { targetName: 'Livongo', acquirerName: 'Teladoc', dealValue: 18500, dealDate: '2020-08', sector: 'digital_therapeutics', stage: 'public', revenueMultiple: 4.1 },
    { targetName: 'Maven Clinic', acquirerName: 'Unknown', dealValue: 0, dealDate: '2023-01', sector: 'maternal_health', stage: 'series_d', revenueMultiple: 3.8 },
    { targetName: 'Tia', acquirerName: 'CVS Health', dealValue: 0, dealDate: '2023-06', sector: 'womens_health', stage: 'series_b', revenueMultiple: 3.5 }
  ];
