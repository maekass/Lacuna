...
function calculateSectorEffects(
  ...
  const grandMean = data.filter(d => d.multiple).reduce((sum, d) => sum + d.multiple!, 0) / 
...
export function analyzeValuationCorrelation(
  ...
  if (n < 3) {
    return {
      ...
      evidencePremium: 0,
      regression: { intercept: 0, evidenceSlope: 0, sectorEffects: new Map(), stageEffects: new Map() },
      insights: ['Need at least 3 acquisitions with known multiples to analyze correlation'],
      recommendations: ['Collect more acquisition data with disclosed valuations']
    };
  }
  ...
  // Segment analysis
  const highEvidence = validData.filter(d => d.evidenceScore.overallScore >= 75);
  const lowEvidence = validData.filter(d => d.evidenceScore.overallScore < 25);
  
  const highEvidenceMultiple = highEvidence.length > 0
    ? highEvidence.reduce((sum, d) => sum + d.multiple!, 0) / highEvidence.length
    : 0;
...
function generateInsights(
  ...
  if (correlation > 0.5) {
    insights.push(`Strong positive correlation (r = ${correlation.toFixed(2)}): Higher evidence maturity predicts higher multiples`);
  } else if (correlation > 0.3) {
    insights.push(`Moderate positive correlation (r = ${correlation.toFixed(2)}): Evidence maturity associated with valuation premium`);
  } else if (correlation > 0) {
    insights.push(`Weak positive correlation (r = ${correlation.toFixed(2)}): Evidence-valuation relationship exists but is noisy`);
  } else {
    insights.push(`No evidence-valuation correlation detected (r = ${correlation.toFixed(2)})`);
  }
  
  if (premium > 2) {
    insights.push(`Evidence premium: ${premium.toFixed(1)}x multiple for high vs low evidence companies`);
  }
  
  insights.push(`Sample composition: ${highEvidenceCount} high evidence, ${lowEvidenceCount} low evidence (n=${totalCount})`);
...
function generateRecommendations(
  ...
  if (correlation > 0.5) {
    recs.push('Prioritize companies with Phase 2+ trials for premium valuations');
    recs.push('Factor evidence score into valuation models - each 10-point increase adds ~' + 
      (slope * 10).toFixed(1) + 'x multiple');
  }
  
  if (premium < 1.5 && correlation > 0.3) {
    recs.push('Evidence premium appears compressed - potential alpha in evidence-rich targets');
  }
  
  recs.push('Monitor clinical trial readouts as leading indicators of valuation inflection points');
...
function interpretCorrelation(r: number): string {
  ...
  if (absR >= 0.7) return 'Strong ' + (r > 0 ? 'positive' : 'negative') + ' correlation';
  if (absR >= 0.5) return 'Moderate ' + (r > 0 ? 'positive' : 'negative') + ' correlation';
  if (absR >= 0.3) return 'Weak ' + (r > 0 ? 'positive' : 'negative') + ' correlation';
...
export function analyzeBySegment(
  ...
  for (const [segment, companies] of segments) {
    ...
    const avgEvidence = validData.reduce((sum, d) => sum + d.evidenceScore.overallScore, 0) / validData.length;
    const avgMultiple = validData.reduce((sum, d) => sum + d.multiple!, 0) / validData.length;
  ...
  return results.sort((a, b) => b.evidenceMultipleCorrelation - a.evidenceMultipleCorrelation);
}
...
export function predictMultiple(
  ...
  let prediction = regression.intercept + regression.evidenceSlope * evidenceScore;
