...
export class OpenFDAClient {
  ...
  async searchDevices(
    ...
    const url = `${this.baseUrl}/device/510k.json?search=${
  ...
  async searchPMA(
    ...
    const query = this.buildCompanyQuery(companyName);
    const url = `${this.baseUrl}/device/pma.json?search=${
      encodeURIComponent(query)
    }&limit=${limit}`;
  ...
  async searchDeNovo(
    ...
    const query = this.buildCompanyQuery(companyName);
    const url = `${this.baseUrl}/device/denovo.json?search=${
      encodeURIComponent(query)
    }&limit=${limit}`;
  ...
  async searchDrugs(
    ...
    const query = this.buildCompanyQuery(companyName);
    const url = `${this.baseUrl}/drug/drugsfda.json?search=${
      encodeURIComponent(query)
    }&limit=${limit}`;
  ...
  private buildCompanyQuery(companyName: string): string {
    ...
    return variants.map((v) => `applicant:"${v}" OR sponsor_name:"${v}"`).join(
      " OR ",
    );
  }
  ...
  private parseDevices(results: unknown[]): FDAProduct[] {
    return results.map((raw) => {
      ...
      return {
        ...
        productName: this.asString(r.device_name) ||
          this.asString(openfda.device_name) || "Unknown",
        companyName: this.asString(r.applicant) ||
          this.asString(r.sponsor_name),
        type: "device",
        clearanceType: "510k",
        approvalDate: this.asString(r.date_received) ||
          this.asString(r.decision_date),
        productClass: this.parseDeviceClass(openfda.device_class),
        indications: this.asString(openfda.indications_for_use)
          ? [this.asString(openfda.indications_for_use)]
          : [],
        devicePanel: this.asOptionalString(
  ...
  private parsePMA(results: unknown[]): FDAProduct[] {
    return results.map((raw) => {
      ...
      return {
        ...
        indications: this.asArray(r.supplement_reason).map((x) =>
          this.asString(x)
        ).filter(Boolean),
      };
    });
  ...
  private parseDrugs(results: unknown[]): FDAProduct[] {
    return results.flatMap((raw) => {
      const r = this.asRecord(raw);
      const submissions = this.asArray(r.submissions).map((s) =>
        this.asRecord(s)
      );
      const firstProduct = this.asRecord(this.asArray(r.products)[0]);
      const firstAi = this.asRecord(
        this.asArray(firstProduct.active_ingredients)[0],
      );

      return submissions.map((s) => ({
        productId: this.asString(s.application_number) || this.asString(r.id),
        productName: this.asString(firstProduct.brand_name) ||
          this.asString(firstProduct.generic_name) ||
          "Unknown",
        companyName: this.asString(r.sponsor_name),
        type: "drug",
        clearanceType: this.parseDrugType(
          s.submission_type ?? s.application_number,
        ),
        approvalDate: this.asString(s.submission_status_date) ||
          this.asString(s.submission_class_date),
  ...
  private getEarliestDate(products: FDAProduct[]): string | undefined {
    ...
    const dates = products.map((p) => new Date(p.approvalDate)).filter((d) =>
      !isNaN(d.getTime())
    );
...
export function calculateFDAMaturityScore(profile: CompanyFDAProfile): number {
  ...
  const classScores = profile.productClasses.map((c) =>
    PRODUCT_CLASS_SCORES[c] || 40
  );
  ...
  if (profile.latestApprovalDate) {
    const yearsSinceApproval =
      (Date.now() - new Date(profile.latestApprovalDate).getTime()) /
      (1000 * 60 * 60 * 24 * 365);
    recencyScore = Math.max(0, 100 - (yearsSinceApproval * 20));
  }

  return Math.round(
    clearanceScore * 0.4 + avgClassScore * 0.2 + portfolioScore * 0.2 +
      recencyScore * 0.2,
  );
}
