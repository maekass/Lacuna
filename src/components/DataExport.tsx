...
export default function DataExport() {
  const { verifiedCompanies, verifiedAcquisitions, verifiedAcquirers, dataProvenance } = useVerifiedDataset();
  const [selectedFormat, setSelectedFormat] = useState<ExportFormat>("json");
  const [isExporting, setIsExporting] = useState(false);
  const [lastExport, setLastExport] = useState<{ format: string; recordCount: number; timestamp: Date } | null>(null);
  ...
  const handleExport = useCallback(() => {
    ...
    if (selectedFormat === "json") {
      ...
      content = JSON.stringify({
        format: "parquet",
        note: "Parquet export requires server-side processing. Use JSON or CSV for direct download.",
        recordCount: verifiedCompanies.length + verifiedAcquisitions.length,
        schema: {
  ...
  }, [selectedFormat, verifiedCompanies, verifiedAcquisitions, verifiedAcquirers, dataProvenance]);

  return (
    <div className="space-y-4">
      <Card>
        <div className="mb-4">
          <h3 className="text-lg font-semibold text-lacuna-plum">Data Export</h3>
        ...
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="bg-slate-50 rounded-lg p-3 border border-slate-200">
            <div className="text-2xl font-bold text-slate-700">{verifiedCompanies.length}</div>
            <div className="text-xs text-slate-600">Companies</div>
          </div>
          <div className="bg-slate-50 rounded-lg p-3 border border-slate-200">
            <div className="text-2xl font-bold text-slate-700">{verifiedAcquisitions.length}</div>
            <div className="text-xs text-slate-600">Acquisitions</div>
          </div>
          <div className="bg-slate-50 rounded-lg p-3 border border-slate-200">
            <div className="text-2xl font-bold text-slate-700">{verifiedAcquirers.length}</div>
          ...
          {EXPORT_OPTIONS.map((option) => (
            ...
            >
              <div className={`p-2 rounded-lg ${
                selectedFormat === option.format ? "bg-lacuna-plum text-white" : "bg-slate-100 text-slate-600"
              }`}>
                {option.icon}
              </div>
              <div className="flex-1">
                <div className="font-medium text-lacuna-plum">{option.label}</div>
                <div className="text-xs text-lacuna-blue">{option.description}</div>
              </div>
              {selectedFormat === option.format && (
        ...
        >
          <DownloadIcon />
          {isExporting ? "Preparing..." : `Export as ${selectedFormat.toUpperCase()}`}
        </button>
        ...
        {lastExport && (
          ...
          >
            <p className="text-sm text-emerald-800">
              <strong>Last export:</strong> {lastExport.format} · {lastExport.recordCount} records ·{" "}
        ...
        <div className="mt-4 p-3 bg-slate-50 rounded-lg border border-slate-200">
          <p className="text-xs text-slate-600">
            <strong>Dataset Schema:</strong> Companies (id, name, sector, stage, description, funding), 
            Acquisitions (id, target, acquirer, value, announcedDate, closedDate, strategicRationale).
          </p>
          <p className="text-xs text-slate-500 mt-2">
            Version: {dataProvenance.datasetVersion} · Updated: {dataProvenance.lastUpdated}
          </p>
        </div>
      </Card>
    </div>
