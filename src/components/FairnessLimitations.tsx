...
import { powerAnalysis, cohenH } from '@/lib/fairness/statisticalMethods';
...
const FAIRNESS_METRIC_INCOMPATIBILITY = [
  ...
  },
  {
    ...
    conflicts: 'Mathematically incompatible with Demographic Parity (Kleinberg 2016)'
  },
  {
    metric: 'Calibration',
...
export default function FairnessLimitations({ 
  ...
  const [activeSection, setActiveSection] = useState<'power' | 'metrics' | 'selection' | 'causal' | null>(null);

  // Power analysis for different effect sizes
  const powerByEffect = [0.1, 0.15, 0.2, 0.25, 0.3, 0.4, 0.5].map(effectSize => {
  ...
  return (
    ...
    >
      ...
      <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg">
        <h3 className="text-xl font-light tracking-tight text-red-800" style={{ fontFamily: "'Bodoni MT', Didot, serif", textTransform: 'uppercase' }}>
          What This Audit CANNOT Tell You
        </h3>
        <p className="text-xs tracking-widest text-red-600 mt-1" style={{ fontFamily: "'Arial Narrow', sans-serif", textTransform: 'uppercase' }}>
      ...
      <div className="grid grid-cols-2 gap-4">
        <button
          onClick={() => setActiveSection(activeSection === 'power' ? null : 'power')}
        ...
        >
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-medium text-red-800" style={{ fontFamily: "'Arial Narrow', sans-serif" }}>
              Heterogeneous Treatment Effects
            </span>
          </div>
          <p className="text-xs text-red-600">
            With n={sampleSize}, we have only {(observedPower.power * 100).toFixed(0)}% power 
        ...
        >
          ...
          <p className="text-xs text-red-600">
            Mathematically impossible per Kleinberg et al. (2016) when base rates differ
          </p>
        </button>
        ...
        <button
          onClick={() => setActiveSection(activeSection === 'causal' ? null : 'causal')}
          className={`p-4 rounded-lg border-2 text-left transition-all ${
      ...
      {activeSection === 'power' && (
        ...
        >
          ...
          <p className="text-sm text-gray-600 mb-4">
            With n={sampleSize} companies, what effect sizes can we reliably detect?
          </p>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {powerByEffect.map(({ effectSize, power, recommendedSampleSize, interpretation }) => {
                const h = cohenH(baselineRate, baselineRate + effectSize);
                return (
                  <tr key={effectSize} className="border-b border-gray-100">
                    <td className="py-2">+{(effectSize * 100).toFixed(0)}pp</td>
                    <td className="py-2 text-gray-500">
                      {h.h.toFixed(2)} ({h.magnitude})
                    </td>
                    <td className="py-2 text-right">
                      <span className={power >= 0.8 ? 'text-green-600' : power >= 0.5 ? 'text-yellow-600' : 'text-red-600'}>
                        {(power * 100).toFixed(0)}%
                      </span>
                    </td>
                    <td className="py-2 text-right text-gray-500">
                      {recommendedSampleSize < 10000 ? recommendedSampleSize : '∞'}
                    </td>
                    <td className="py-2 pl-4 text-xs text-gray-600">{interpretation}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      {activeSection === 'metrics' && (
        ...
        >
          ...
          <p className="text-sm text-gray-600 mb-4">
            Kleinberg, Mullainathan, and Raghavan (2016) proved that three popular fairness 
            definitions cannot simultaneously be satisfied when base rates differ between groups.
          </p>

          <div className="space-y-3">
            {FAIRNESS_METRIC_INCOMPATIBILITY.map(m => (
              <div key={m.metric} className="border border-gray-200 rounded-lg p-3">
                <div className="font-medium text-sm mb-1">{m.metric}</div>
                <div className="text-xs font-mono bg-gray-50 p-2 rounded mb-2">{m.definition}</div>
          ...
          <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded text-sm text-blue-800">
            ...
            We explicitly acknowledge this choice trades off against other fairness properties.
          </div>
        </motion.div>
      )}
      ...
      {activeSection === 'selection' && (
        ...
        >
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-4">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-red-600">Unobserved: Failed companies (n=?)</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-6 h-6 bg-red-500 rounded-full flex items-center justify-center text-white text-xs">✗</div>
                <span className="text-red-600">Unobserved: Companies that never got funded</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-6 h-6 bg-red-500 rounded-full flex items-center justify-center text-white text-xs">✗</div>
                <span className="text-red-600">Unobserved: Pipeline (pre-Series A)</span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded text-sm text-blue-800">
            <strong>Honest disclosure:</strong> To properly test for discrimination, we would need:
            <ul className="mt-1 ml-4 space-y-0.5">
      ...
      {activeSection === 'causal' && (
        ...
        >
          ...
          <div className="p-3 bg-gray-100 rounded text-sm text-gray-700">
            <strong>Why the distinction matters:</strong> Causal claims require either:
            <ul className="mt-1 ml-4 space-y-0.5">
      ...
      <div className="bg-gradient-to-br from-red-100 to-amber-100 p-6 rounded-lg border border-red-300">
        <h4 className="font-medium text-red-900 mb-3" style={{ fontFamily: "'Arial Narrow', sans-serif", textTransform: 'uppercase', letterSpacing: '0.1em' }}>
          Honest Methodological Confession
        </h4>
        <p className="text-sm text-red-800 leading-relaxed">
          This fairness audit is fundamentally limited. With n={sampleSize} acquired companies, 
          gender inference at {Math.round(94)}% accuracy, and no observation of failures or pipeline data, 
          we cannot make definitive claims about fairness, discrimination, or bias.
          <br/><br/>
