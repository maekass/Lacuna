...
function extractCapabilities(description: string): string[] {
  ...
  if (description.includes('mental') || description.includes('therapy')) caps.push('mental health');
  if (description.includes('coaching') || description.includes('care')) caps.push('care coordination');
...
export default function AcquirerPredictionDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {companyProfiles.map(company => (
            ...
            >
              {company.name}
              <span className="ml-2 text-xs opacity-75">({company.sector})</span>
            </button>
          ))}
        </div>
      </div>
        ...
        >
          ...
          <div className="bg-white rounded-lg shadow-lg p-6">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-xl font-bold text-lacuna-plum">{selectedAnalysis.company.name}</h3>
                <p className="text-sm text-lacuna-blue">
                  {selectedAnalysis.company.sector.replace(/_/g, ' ')} • {selectedAnalysis.company.stage.replace(/_/g, ' ')}
                </p>
              </div>
            ...
            <div className="grid grid-cols-3 gap-4 mt-4 pt-4 border-t border-gray-100">
              <div className="text-center">
                <div className="text-lg font-semibold text-green-600">
                  {selectedAnalysis.topMatches.filter(m => m.likelihood === 'high').length}
                </div>
                <div className="text-xs text-gray-500">High Interest Acquirers</div>
              </div>
              <div className="text-center">
                <div className="text-lg font-semibold text-lacuna-plum">
                  {formatCurrency(selectedAnalysis.fairValueEstimate.median)}
                </div>
                <div className="text-xs text-gray-500">Estimated Fair Value</div>
              </div>
              <div className="text-center">
                <div className="text-lg font-semibold text-orange-600">
                  {selectedAnalysis.estimatedBiddingWarPremium > 0 ? `+${selectedAnalysis.estimatedBiddingWarPremium}%` : 'None'}
                </div>
            ...
            {selectedAnalysis.predictedWinner && (
              <div className="mt-4 p-3 bg-green-50 rounded-lg border border-green-200">
                ...
                <div className="text-xs text-green-600">
                  ...
                  Estimated Value: {formatCurrency(selectedAnalysis.topMatches[0].estimatedValue.median)}
                </div>
              </div>
            )}
          ...
          <div className="bg-white rounded-lg shadow-lg overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100">
              <h4 className="font-semibold text-lacuna-plum">Acquirer Rankings</h4>
            </div>
            <div className="divide-y divide-gray-100">
              {selectedAnalysis.topMatches.map((match, idx) => (
                <div key={match.acquirer.id} className="px-6 py-4 hover:bg-gray-50">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className="text-2xl font-bold text-gray-300 w-8">#{idx + 1}</div>
                      <div>
                        <div className="font-medium text-lacuna-plum">{match.acquirer.name}</div>
                        <div className="text-xs text-lacuna-blue">
                          {match.acquirer.type.replace(/_/g, ' ')} • 
                          Deal size: {formatCurrency(match.acquirer.typicalDealSize.min)}-{formatCurrency(match.acquirer.typicalDealSize.max)}
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <div className="text-lg font-semibold text-lacuna-plum">{match.matchScore}</div>
                        <div className="text-xs text-gray-500">Match Score</div>
                      </div>
                      <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getLikelihoodColor(match.likelihood)}`}>
                  ...
                  <div className="mt-3 grid grid-cols-4 gap-4 text-center">
                    <div className="p-2 bg-gray-50 rounded">
                      <div className="text-sm font-semibold text-lacuna-plum">{match.strategicFit}</div>
                      <div className="text-xs text-gray-500">Strategic Fit</div>
                    </div>
                    <div className="p-2 bg-gray-50 rounded">
                      <div className="text-sm font-semibold text-lacuna-plum">{match.marketFit}</div>
                      <div className="text-xs text-gray-500">Market Fit</div>
                    </div>
                    <div className="p-2 bg-gray-50 rounded">
                      <div className="text-sm font-semibold text-lacuna-plum">{match.financialFit}</div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h4 className="font-semibold text-lacuna-plum mb-3">Exit Timeline Estimate</h4>
            <div className="flex items-center gap-4">
            ...
            <div className="mt-4">
              <div className="text-sm font-medium text-gray-700 mb-2">Events that could accelerate:</div>
              <div className="flex flex-wrap gap-2">
                {selectedAnalysis.timelineEstimate.triggers.map((trigger, i) => (
                  <span key={i} className="text-xs px-3 py-1 bg-orange-50 text-orange-700 rounded-full border border-orange-200">
                    {trigger}
                  </span>
                ))}
              </div>
          ...
          {/* Sector Comparables */}
          <div className="bg-white rounded-lg shadow p-6">
            <h4 className="font-semibold text-lacuna-plum mb-3">Recent Sector Comparables</h4>
            <div className="space-y-2">
              {selectedAnalysis.sectorComparables.slice(0, 3).map((deal, i) => (
                <div key={i} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                  <div>
                    <span className="font-medium text-sm">{deal.targetName}</span>
                    <span className="text-gray-500 text-xs ml-2">→ {deal.acquirerName}</span>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-medium">{formatCurrency(deal.dealValue)}</div>
                    {deal.revenueMultiple && (
                      <div className="text-xs text-gray-500">{deal.revenueMultiple}x revenue</div>
                    )}
                  </div>
                </div>
              ))}
      ...
      {!selectedCompany && (
        <div className="bg-white rounded-lg shadow-lg p-6">
          <h4 className="font-semibold text-lacuna-plum mb-4">All Companies - Acquirer Interest Overview</h4>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-lacuna-lavender/10">
                <tr>
                  <th className="px-4 py-2 text-left text-xs font-semibold">Company</th>
                  <th className="px-4 py-2 text-left text-xs font-semibold">Sector</th>
                  <th className="px-4 py-2 text-center text-xs font-semibold">Top Acquirer</th>
                  <th className="px-4 py-2 text-center text-xs font-semibold">Match</th>
                  <th className="px-4 py-2 text-right text-xs font-semibold">Est. Value</th>
                  <th className="px-4 py-2 text-center text-xs font-semibold">Interest Level</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {analyses.map(({ profile, analysis }) => (
                  ...
                  >
                    <td className="px-4 py-3 font-medium text-lacuna-plum">{profile.name}</td>
                    <td className="px-4 py-3 text-sm text-lacuna-blue">{profile.sector.replace(/_/g, ' ')}</td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${getLikelihoodColor(analysis.topMatches[0]?.likelihood || 'low')}`}>
                        {analysis.topMatches[0]?.likelihood?.toUpperCase() || 'LOW'}
                      </span>
                    </td>
                  </tr>
                ))}
