...
export const BusinessModelClassifier: React.FC<BusinessModelClassifierProps> = ({
  ...
  const [hasCPTCode, setHasCPTCode] = useState(companyProfile?.reimbursementStatus.hasCPTCode ?? false);
  const [codeCount, setCodeCount] = useState(companyProfile?.reimbursementStatus.codeCount ?? 0);
  const [rateCategory, setRateCategory] = useState<ReimbursementStatus['rateCategory']>(
    companyProfile?.reimbursementStatus.rateCategory ?? 'none'
  );
  const [revenue, setRevenue] = useState(annualRevenue);
  const [acquirerType, setAcquirerType] = useState<'healthcare' | 'tech' | 'pharma' | 'retail'>('healthcare');
  ...
  const classification = useMemo<ClassificationResult>(() => {
    const reimbursementStatus: ReimbursementStatus = {
      ...
      codeCount,
      reimbursementBreadth: codeCount > 1 ? 'multi-payer' : hasCPTCode ? 'medicare-only' : 'none',
      rateCategory,
  ...
  }, [hasCPTCode, codeCount, rateCategory, revenue, selectedSector, acquirerType, calculator]);
  ...
  const getBusinessModelLabel = (model: BusinessModel): string => {
    switch (model) {
      case 'insurance-driven': return 'Insurance-Driven (B2B)';
      case 'hybrid': return 'Hybrid Model';
      case 'b2c-consumer': return 'Consumer-Only (B2C)';
  ...
  const getBusinessModelColor = (model: BusinessModel): string => {
    switch (model) {
      case 'insurance-driven': return 'bg-green-100 text-green-800 border-green-300';
      case 'hybrid': return 'bg-blue-100 text-blue-800 border-blue-300';
      case 'b2c-consumer': return 'bg-orange-100 text-orange-800 border-orange-300';
    ...
    <div className={`bg-white rounded-lg shadow-lg p-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div>
          ...
          <input
            ...
            className="w-full"
          />
          <div className="text-sm text-gray-600 text-center">{formatCurrency(revenue)}</div>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Has CPT Code</label>
        ...
        {hasCPTCode && (
          <>
            ...
            <div>
              ...
              <select
                value={rateCategory}
                onChange={(e) => setRateCategory(e.target.value as ReimbursementStatus['rateCategory'])}
                className="w-full border border-gray-300 rounded-md px-3 py-2"
              >
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Likely Acquirer</label>
          <select
            value={acquirerType}
            onChange={(e) => setAcquirerType(e.target.value as typeof acquirerType)}
            className="w-full border border-gray-300 rounded-md px-3 py-2"
          >
      ...
      <div className="bg-gray-50 rounded-lg p-4 mb-6">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className={`px-3 py-1 rounded-full text-sm font-semibold border ${getBusinessModelColor(classification.businessModel)}`}>
      ...
      {/* Model Comparison */}
      <div className="mb-6">
        <h3 className="font-semibold text-gray-900 mb-3">Insurance-Driven vs Consumer-Only Comparison</h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-green-50 rounded-lg p-4 border border-green-200">
            <h4 className="font-semibold text-green-900 mb-2">Insurance-Driven Model</h4>
          ...
          <div className="bg-orange-50 rounded-lg p-4 border border-orange-200">
            <h4 className="font-semibold text-orange-900 mb-2">Consumer-Only Model</h4>
        ...
        <div className="mt-4 bg-purple-50 rounded-lg p-4 border border-purple-200">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm font-medium text-purple-900">Reimbursement Premium</div>
              <div className="text-2xl font-bold text-purple-700">
                +{modelComparison.premiumPercent.toFixed(0)}%
              </div>
            </div>
            <div className="text-right">
              <div className="text-sm font-medium text-purple-900">Additional Value</div>
          ...
          <div className="mt-2 text-xs text-purple-600">
            Companies with strong reimbursement profiles command 3-5x premiums over consumer-only models
          </div>
        </div>
      </div>
