...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    case 'DISABLED':
      return { ...state, status: 'disabled', message: 'Variant store is off on this deployment.' };
    case 'LOADING':
    ...
    case 'SELECT':
      return { ...state, selectedId: action.callsetId, variants: [], total: 0, presignedUrl: null };
    case 'GENE':
...
export default function VariantCallsetBrowser() {
  ...
  useEffect(() => {
    ...
    fetch('/api/genomics/callsets?limit=25')
      ...
      .catch(() => dispatch({ type: 'ERROR', message: 'Could not load callsets.' }));
  }, []);

  useEffect(() => {
    if (!state.selectedId || state.status !== 'ready') return;

    const params = new URLSearchParams({ callsetId: state.selectedId, limit: '50' });
    if (state.geneFilter.trim()) params.set('gene', state.geneFilter.trim().toUpperCase());

    fetch(`/api/genomics/variants?${params}`)
      .then((res) => (res.ok ? res.json() : Promise.reject(new Error('variants failed'))))
      .then((data) =>
        dispatch({
          type: 'VARIANTS',
      ...
      .catch(() => dispatch({ type: 'ERROR', message: 'Could not load variants for callset.' }));

    fetch(`/api/genomics/callsets/${encodeURIComponent(state.selectedId)}/object`)
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => dispatch({ type: 'PRESIGN', url: data?.presignedUrl ?? null }));
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-lacuna-lavender/40 p-4 sm:p-6">
      ...
      <h3 className="text-lg font-semibold text-lacuna-plum mb-1">Variant call-set browser</h3>
      <p className="text-sm text-lacuna-blue mb-4">
        Queryable variant summaries from ClickHouse; multi-GB VCF blobs stay in object storage.
      </p>
      ...
      {state.status === 'ready' && (
        <>
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            <label className="flex-1 text-sm text-lacuna-blue">
              ...
              <select
                ...
                onChange={(e) => dispatch({ type: 'SELECT', callsetId: e.target.value })}
              >
            ...
            <label className="sm:w-48 text-sm text-lacuna-blue">
              ...
              <input
                ...
                onChange={(e) => dispatch({ type: 'GENE', gene: e.target.value })}
              />
            </label>
          </div>
          ...
          <p className="text-xs text-lacuna-blue/80 mb-2">
            Showing {state.variants.length} of {state.total.toLocaleString()} matching variants
          </p>
            ...
            <table className="min-w-full text-xs">
              ...
              <tbody>
                {state.variants.map((v) => (
                  <tr key={`${v.chrom}-${v.pos}-${v.ref}-${v.alt}`} className="border-t border-slate-100">
                    ...
                    <td className="px-2 py-1.5 truncate max-w-[12rem]">{v.consequence || '—'}</td>
                    <td className="px-2 py-1.5">{v.isPathogenic ? 'Yes' : 'No'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {state.status === 'loading' && (
        <div className="h-32 animate-pulse rounded-lg bg-lacuna-pink/10" aria-hidden />
      )}
    </div>
  );
}
