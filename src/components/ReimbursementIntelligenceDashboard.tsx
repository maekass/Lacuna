...
export default function ReimbursementIntelligenceDashboard() {
  ...
  const getBusinessModelLabel = (model: BusinessModel): string => {
    switch (model) {
      case 'insurance-driven': return 'Insurance-Driven';
      case 'hybrid': return 'Hybrid';
      case 'b2c-consumer': return 'Consumer-Only';
    ...
    return (
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="flex items-center justify-center h-64">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-lacuna-plum"></div>
          <span className="ml-3 text-lacuna-blue">Analyzing reimbursement data...</span>
        </div>
      </div>
    );
  }
      ...
      {stats && (
        ...
        >
          <div className="bg-white rounded-lg p-4 shadow-sm border border-lacuna-lavender/20">
            <div className="text-2xl font-bold text-lacuna-plum">{stats.totalCompanies}</div>
            <div className="text-xs text-lacuna-blue">Companies Analyzed</div>
          </div>
          <div className="bg-green-50 rounded-lg p-4 shadow-sm border border-green-200">
            <div className="text-2xl font-bold text-green-700">{stats.insuranceDriven}</div>
            <div className="text-xs text-green-700">Insurance-Driven</div>
          </div>
          <div className="bg-blue-50 rounded-lg p-4 shadow-sm border border-blue-200">
            <div className="text-2xl font-bold text-blue-700">{stats.hybrid}</div>
          ...
          <div className="bg-orange-50 rounded-lg p-4 shadow-sm border border-orange-200">
            <div className="text-2xl font-bold text-orange-700">{stats.consumerOnly}</div>
      ...
      {stats && (
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm font-medium text-purple-900">Average Valuation Multiple</div>
              <div className="text-3xl font-bold text-purple-700">{formatMultiple(stats.avgValuationMultiple)}</div>
            </div>
            <div className="text-right">
              <div className="text-sm font-medium text-purple-900">Reimbursement Premium Range</div>
              <div className="text-xl font-bold text-purple-700">
                {stats.reimbursementPremiumRange.min.toFixed(1)}x - {stats.reimbursementPremiumRange.max.toFixed(1)}x
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-lacuna-lavender/10">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-semibold text-lacuna-plum uppercase">Company</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-lacuna-plum uppercase">Sector</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-lacuna-plum uppercase">Business Model</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-lacuna-plum uppercase">CPT Codes</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-lacuna-plum uppercase">Est. Insurance Rev</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-lacuna-plum uppercase">Valuation Multiple</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-lacuna-plum uppercase">Implied Value</th>
              </tr>
            </thead>
              ...
              {filteredAnalyses.map((analysis, idx) => (
                ...
                >
                  <td className="px-4 py-3">
                    <div className="font-medium text-lacuna-plum">{analysis.company.name}</div>
                    <div className="text-xs text-lacuna-blue truncate max-w-xs">
                      {analysis.profile.matchedCodes.slice(0, 2).map(c => c.code).join(', ')}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded text-xs font-medium border ${
                      SECTOR_COLORS[analysis.company.sector] || 'bg-gray-100 text-gray-800'
                    }`}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded text-xs font-medium border ${
                      BUSINESS_MODEL_COLORS[analysis.classification.businessModel]
                    }`}>
                      {getBusinessModelLabel(analysis.classification.businessModel)}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-sm text-lacuna-blue">
                    {analysis.classification.estimatedInsuranceRevenue > 0 
                      ? `${(analysis.classification.estimatedInsuranceRevenue / (analysis.company.revenue || 1) * 100).toFixed(0)}%`
                      : '0%'}
                  </td>
                  <td className="px-4 py-3">
                    <span className={`font-semibold ${
                      analysis.valuation.adjustedMultiple > 4 ? 'text-green-600' :
                      analysis.valuation.adjustedMultiple > 2.5 ? 'text-blue-600' : 'text-orange-600'
                    }`}>
        ...
        >
          ...
          <div className="space-y-3">
            {filteredAnalyses.slice(0, 3).map((analysis, idx) => (
              <div key={idx} className="flex items-start gap-3 p-3 bg-lacuna-lavender/10 rounded-lg">
                <span className="text-lacuna-plum font-medium">{analysis.company.name}:</span>
                <span className="text-sm text-lacuna-blue">{analysis.insights[0]}</span>
              </div>
            ))}
          </div>
        </motion.div>
