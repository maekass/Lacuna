...
import { 
  CompanyTrialProfile 
} from '@/data/clinicaltrials-mcp-connector';
import {
  CompanyFDAProfile
} from '@/data/openfda-mcp-connector';
...
export default function EvidenceTimeline({
  ...
  const events = useMemo(() => {
    ...
    // Publication events
    trialProfile.trials.filter(t => t.hasPublishedResults).forEach(trial => {
      // Estimate publication date (6 months after completion)
      const completion = trial.completionDate ? new Date(trial.completionDate) : new Date();
      ...
      allEvents.push({
        date: pubDate,
        type: 'publication',
        title: trial.isFlagshipJournal ? 'Flagship Journal Publication' : 'Publication',
  ...
  useEffect(() => {
    ...
    // Add padding
    const timePadding = (timeExtent[1].getTime() - timeExtent[0].getTime()) * 0.1;
    ...
    eventGroup.append('circle')
      ...
      .attr('fill', d => {
        ...
        if (d.type === 'publication') {
          return d.journal?.includes('Nature') || d.journal?.includes('NEJM') || d.journal?.includes('JAMA')
            ? '#dc2626' // Flagship
            : '#3b82f6'; // Regular
        }
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-lg font-semibold text-lacuna-plum">{companyName}</h3>
          <p className="text-sm text-lacuna-blue">
            Evidence Maturity Score: <span className="font-semibold">{evidenceScore.overallScore}/100</span>
          </p>
        </div>
        <div className="text-right">
          <div className={`inline-flex px-3 py-1 rounded-full text-xs font-medium ${
            evidenceScore.overallScore >= 75 ? 'bg-green-100 text-green-800' :
            evidenceScore.overallScore >= 50 ? 'bg-blue-100 text-blue-800' :
            evidenceScore.overallScore >= 25 ? 'bg-yellow-100 text-yellow-800' :
      ...
      <div className="grid grid-cols-4 gap-4 mt-4 pt-4 border-t border-gray-100">
        <div className="text-center">
          <div className="text-lg font-bold text-lacuna-plum">{trialProfile.totalTrials}</div>
          <div className="text-xs text-lacuna-blue">Trials</div>
        </div>
        <div className="text-center">
          <div className="text-lg font-bold text-lacuna-plum">{fdaProfile.totalProducts}</div>
          <div className="text-xs text-lacuna-blue">FDA Products</div>
        </div>
        <div className="text-center">
          <div className="text-lg font-bold text-lacuna-plum">{evidenceScore.phaseScore}</div>
          <div className="text-xs text-lacuna-blue">Phase Score</div>
        </div>
        <div className="text-center">
          <div className="text-lg font-bold text-lacuna-plum">{evidenceScore.fdaStatusScore}</div>
      ...
      {acquisitionDate && (
        <div className="mt-4 p-3 bg-lacuna-lavender/10 rounded-lg">
          <div className="text-xs font-medium text-lacuna-plum mb-1">At Acquisition</div>
          <div className="text-sm text-lacuna-blue">
            {evidenceScore.atAcquisition.description}
          </div>
          <div className="text-xs text-lacuna-blue mt-1">
            Evidence Score: {evidenceScore.atAcquisition.evidenceScore}/100 | 
            Phase: {evidenceScore.atAcquisition.phase} | 
            FDA: {evidenceScore.atAcquisition.hadFDAApproval ? 'Yes' : 'No'}
          </div>
        </div>
      )}
    </div>
