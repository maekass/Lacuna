...
import GenderInferenceQuality, {
  type FounderClassification,
} from "./GenderInferenceQuality";
import FounderCharacteristics, {
  type CompanyProfile,
} from "./FounderCharacteristics";
import {
  ...
  const parityAnalysis = useMemo(() => {
    if (!hasFounderGenderLabels) {
      ...
      return {
        ...
        womenRate: 0,
        menRate: sampleCompanies.length > 0
          ? acquired / sampleCompanies.length
          : 0,
        ...
        >
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <ul className="space-y-2 text-sm text-gray-700">
              <li>
                ...
                {" "}
                {(parityAnalysis.womenCI[0] * 100).toFixed(0)}%,{" "}
                {(parityAnalysis.womenCI[1] * 100).toFixed(0)}%]
              </li>
              <li>
                • <strong>Men-founded acquisition rate:</strong>{" "}
                {(parityAnalysis.menRate * 100).toFixed(0)}% [95% Wilson CI:
                {" "}
                {(parityAnalysis.menCI[0] * 100).toFixed(0)}%,{" "}
                {(parityAnalysis.menCI[1] * 100).toFixed(0)}%]
              </li>
              <li>
                • <strong>Difference:</strong>{" "}
                {(parityAnalysis.difference.difference * 100).toFixed(0)}pp{" "}
                [95% Newcombe CI:{" "}
                {(parityAnalysis.difference.lower * 100).toFixed(0)}pp,{" "}
                {(parityAnalysis.difference.upper * 100).toFixed(0)}pp]
              </li>
              <li>
                • <strong>Fisher&apos;s exact test:</strong> p ={" "}
                {parityAnalysis.fisher.pValue.toFixed(3)}{" "}
                ({parityAnalysis.fisher.interpretation})
              </li>
              <li>
                • <strong>Bonferroni-corrected α:</strong>{" "}
                {parityAnalysis.bonferroni.adjustedAlpha.toFixed(3)}
                ({parityAnalysis.bonferroni.numSignificant} of{" "}
          ...
          <div className="bg-gradient-to-r from-[#E8B4B8] via-[#B8A9C9] to-[#4A5D8A] p-6 rounded-lg text-white">
            ...
            <p className="text-sm leading-relaxed">
              ...
              {(parityAnalysis.difference.difference * 100).toFixed(0)}pp is
              {" "}
              <strong>
                {parityAnalysis.fisher.pValue < 0.05 ? "" : "NOT "}statistically
                significant
              </strong>
              (Fisher&apos;s exact p={parityAnalysis.fisher.pValue.toFixed(3)}).
              The confidence interval [{(parityAnalysis.difference.lower * 100)
                .toFixed(0)}pp,{" "}
              {(parityAnalysis.difference.upper * 100).toFixed(0)}pp]
              {parityAnalysis.difference.lower *
                    parityAnalysis.difference.upper > 0
                ? " excludes zero"
                : " includes zero"}.
              <br />
              <br />
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <div>
                      Adjusted α:{" "}
                      <strong>
                        {parityAnalysis.bonferroni.adjustedAlpha.toFixed(4)}
                      </strong>
                    </div>
                    <div>
                      {parityAnalysis.bonferroni.numSignificant} of{" "}
                      {parityAnalysis.bonferroni.numTests} significant
                    </div>
                  </div>
                  <div>
                    ...
                    <div>
                      {parityAnalysis.benjaminiHochberg.numSignificant}{" "}
                      of 5 significant
                    </div>
                  </div>
                </div>
