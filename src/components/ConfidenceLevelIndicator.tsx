...
export default function ConfidenceLevelIndicator({
  ...
  return (
    <div className="inline-flex items-center gap-2 group relative">
      <span 
        className={`${config.bgColor} ${config.textColor} ${sizeClasses[size]} rounded font-medium uppercase tracking-wider`}
...
export function ConfidenceLegend() {
  return (
    <div className="flex flex-wrap gap-3 text-xs" style={{ fontFamily: "'Arial Narrow', sans-serif" }}>
    ...
    <div className={`p-3 rounded-lg border ${config.borderColor} ${config.bgColor}`}>
      <div className="flex items-start justify-between gap-2">
        <div className="flex-1">
          <div className="text-xs text-gray-500 uppercase tracking-wider mb-1" style={{ fontFamily: "'Arial Narrow', sans-serif" }}>
            {label}
          </div>
        ...
        <ConfidenceLevelIndicator level={level} source={source} limitation={limitation} size="sm" />
      </div>
      {(source || limitation) && (
        <div className="mt-2 pt-2 border-t border-gray-200 text-xs space-y-1">
          {source && <p className="text-gray-600"><strong>Source:</strong> {source}</p>}
          {limitation && <p className="text-amber-600"><strong>Limitation:</strong> {limitation}</p>}
        </div>
      )}
    </div>
  );
