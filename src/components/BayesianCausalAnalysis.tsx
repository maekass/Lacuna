...
export default function BayesianCausalAnalysis() {
  ...
  const getProbabilityColor = (prob: number, direction: 'positive' | 'negative'): string => {
    ...
    >
      ...
      <div className="bg-amber-50 border-l-4 border-amber-500 p-4 rounded-r-lg">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className="font-medium text-amber-800" style={{ fontFamily: "'Arial Narrow', sans-serif", textTransform: 'uppercase' }}>
              Small Sample Limitation Acknowledged
            </h4>
            <p className="text-sm text-amber-700 mt-1">
              Current sample (n={inputs.sampleSize}) is underpowered for heterogeneous treatment effects.
              Causal forest requires n≥200. Using Bayesian main effects only with strong priors.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-50 p-4 rounded-lg space-y-4">
        <h4 className="text-sm font-medium tracking-wider uppercase" style={{ fontFamily: "'Arial Narrow', sans-serif" }}>
          Model Parameters
        </h4>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {[
            ...
            { label: 'Prior Variance', key: 'priorVariance', step: 0.1, min: 0.01 }
          ].map((field) => (
            <div key={field.key} className="space-y-1">
              <label className="text-xs text-gray-500 uppercase" style={{ fontFamily: "'Arial Narrow', sans-serif" }}>
                {field.label}
              </label>
              <input
                type="number"
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="px-3 py-1 rounded text-white text-xs font-medium bg-[#5D4E6D]">
            PRIOR INFLUENCE: {(analysis.mainEffects.priorInfluence * 100).toFixed(1)}%
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            <div className="text-3xl font-light mb-1" style={{ fontFamily: "'Bodoni MT', Didot, serif", color: '#5D4E6D' }}>
              {analysis.mainEffects.posteriorMean.toFixed(3)}
            </div>
            <div className="text-xs text-gray-500 uppercase tracking-wider" style={{ fontFamily: "'Arial Narrow', sans-serif" }}>
              Posterior Mean
            </div>
          </div>
          ...
          {/* Credible Interval */}
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            <div className="text-lg font-light mb-1" style={{ fontFamily: "'Bodoni MT', Didot, serif", color: '#4A5D8A' }}>
              [{analysis.mainEffects.credibleInterval[0].toFixed(3)}, {analysis.mainEffects.credibleInterval[1].toFixed(3)}]
            </div>
            <div className="text-xs text-gray-500 uppercase tracking-wider" style={{ fontFamily: "'Arial Narrow', sans-serif" }}>
          ...
          {/* Probabilities */}
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            <div className={`text-2xl font-light mb-1 ${getProbabilityColor(analysis.mainEffects.probabilityPositive, 'positive')}`} style={{ fontFamily: "'Bodoni MT', Didot, serif" }}>
          ...
          {/* Bayes Factor */}
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            <div className="text-2xl font-light mb-1" style={{ fontFamily: "'Bodoni MT', Didot, serif", color: analysis.mainEffects.bayesFactor > 3 ? '#2d6a4f' : analysis.mainEffects.bayesFactor > 1 ? '#e9c46a' : '#e76f51' }}>
        ...
        <div className="mt-4 p-4 bg-gray-50 rounded-lg">
          <p className="text-sm text-gray-700">
            ...
            {analysis.mainEffects.probabilityPositive > 0.8 
              ? `Evidence supports positive effect (P=${(analysis.mainEffects.probabilityPositive * 100).toFixed(1)}%). `
              : analysis.mainEffects.probabilityNegative > 0.8
              ? `Evidence supports negative effect (P=${(analysis.mainEffects.probabilityNegative * 100).toFixed(1)}%). `
              : `Inconclusive evidence (P(positive)=${(analysis.mainEffects.probabilityPositive * 100).toFixed(1)}%, P(negative)=${(analysis.mainEffects.probabilityNegative * 100).toFixed(1)}%). `
            }
            Bayes factor of {analysis.mainEffects.bayesFactor.toFixed(1)} indicates{' '}
            {analysis.mainEffects.bayesFactor > 10 ? 'strong' : analysis.mainEffects.bayesFactor > 3 ? 'moderate' : 'weak'} evidence for effect vs. no effect.
          </p>
          ...
          >
            <div className="bg-green-50 border-l-4 border-green-500 p-3 mb-4 rounded-r">
              <p className="text-sm text-green-800">
                <strong>✓ All hypotheses pre-registered:</strong>{' '}
            ...
            <div className="space-y-3">
              {analysis.preRegisteredResults.map((result) => (
                <div key={result.hypothesis.id} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-600 mt-1">{result.hypothesis.description}</p>
                  ...
                  {result.result && (
                    <div className="mt-3 pt-3 border-t border-gray-100 grid grid-cols-3 gap-4 text-center">
                      <div>
                        <div className="text-lg font-light" style={{ fontFamily: "'Bodoni MT', Didot, serif" }}>
                          {result.result.posteriorMean.toFixed(3)}
                        </div>
                        <div className="text-xs text-gray-500">Posterior Mean</div>
                      </div>
                      <div>
                        <div className={`text-lg font-light ${getProbabilityColor(result.result.probabilityPositive, result.hypothesis.direction === 'positive' ? 'positive' : 'negative')}`}>
                          {result.hypothesis.direction === 'positive' 
                            ? (result.result.probabilityPositive * 100).toFixed(0)
                            : (result.result.probabilityNegative * 100).toFixed(0)}%
                        </div>
                        <div className="text-xs text-gray-500">P(Hypothesis)</div>
                      </div>
                      <div>
                        <div className="text-lg font-light" style={{ fontFamily: "'Bodoni MT', Didot, serif", color: '#5D4E6D' }}>
                          {result.result.bayesFactor.toFixed(1)}
                        </div>
                        <div className="text-xs text-gray-500">Bayes Factor</div>
                      </div>
                    </div>
                  )}
      ...
      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
        <h4 className="font-medium text-red-800 mb-2" style={{ fontFamily: "'Arial Narrow', sans-serif", textTransform: 'uppercase' }}>
      ...
      <div className="bg-gray-800 text-white p-4 rounded-lg">
        ...
        <div className="text-sm space-y-1 text-gray-300">
          ...
          <p className="mt-2 text-white">
            <strong>Instead:</strong> Bayesian main effects with pre-registration and explicit limitations.
          </p>
        </div>
      </div>
    </motion.div>
