...
export default function StrategicPositioningMap({ result }: StrategicPositioningMapProps) {
  const [hoveredAcquirer, setHoveredAcquirer] = useState<StrategicPosition | null>(null);
  const [selectedAcquirer, setSelectedAcquirer] = useState<StrategicPosition | null>(null);
  ...
  const yScale = (y: number) => margin.top + innerHeight - (y / maxVelocity * innerHeight);
    ...
    >
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="relative">
          <svg width={width} height={height} className="overflow-visible max-w-full">
            ...
            <rect x={margin.left} y={margin.top} width={innerWidth/2} height={innerHeight/2}
                  fill="#E8B4B8" opacity={0.08} />
            <rect x={margin.left + innerWidth/2} y={margin.top} width={innerWidth/2} height={innerHeight/2}
                  fill="#4A5D8A" opacity={0.08} />
            <rect x={margin.left} y={margin.top + innerHeight/2} width={innerWidth/2} height={innerHeight/2}
                  fill="#B8A9C9" opacity={0.08} />
            <rect x={margin.left + innerWidth/2} y={margin.top + innerHeight/2} width={innerWidth/2} height={innerHeight/2}
                  fill="#5D4E6D" opacity={0.08} />

            {/* Quadrant Labels */}
            <text x={margin.left + innerWidth/4} y={margin.top + 20} 
                  textAnchor="middle" className="text-xs fill-gray-600" 
                  style={{ fontFamily: "'Arial Narrow', sans-serif", letterSpacing: '0.05em' }}>
              FOCUSED AGGRESSIVE
            </text>
            <text x={margin.left + 3*innerWidth/4} y={margin.top + 20} 
                  textAnchor="middle" className="text-xs fill-gray-600" 
                  style={{ fontFamily: "'Arial Narrow', sans-serif", letterSpacing: '0.05em' }}>
              AGGRESSIVE DIVERSIFIER
            </text>
            <text x={margin.left + innerWidth/4} y={margin.top + innerHeight - 5} 
                  textAnchor="middle" className="text-xs fill-gray-600" 
                  style={{ fontFamily: "'Arial Narrow', sans-serif", letterSpacing: '0.05em' }}>
              SELECTIVE SPECIALIST
            </text>
            <text x={margin.left + 3*innerWidth/4} y={margin.top + innerHeight - 5} 
                  textAnchor="middle" className="text-xs fill-gray-600" 
                  style={{ fontFamily: "'Arial Narrow', sans-serif", letterSpacing: '0.05em' }}>
            ...
            <line x1={margin.left + innerWidth/2} y1={margin.top} 
                  x2={margin.left + innerWidth/2} y2={margin.top + innerHeight} 
                  stroke="#cbd5e1" strokeWidth="1" strokeDasharray="4 4" />
            <line x1={margin.left} y1={margin.top + innerHeight/2} 
                  x2={margin.left + innerWidth} y2={margin.top + innerHeight/2} 
                  stroke="#cbd5e1" strokeWidth="1" strokeDasharray="4 4" />

            {/* X-axis label */}
            <text x={margin.left + innerWidth/2} y={height - 15} 
                  textAnchor="middle" className="text-sm fill-gray-700 font-medium"
                  style={{ fontFamily: "'Arial Narrow', sans-serif", letterSpacing: '0.08em' }}>
              SECTOR BREADTH (SPECIALIST → GENERALIST)
            </text>
            
            {/* Y-axis label */}
            <text x={15} y={margin.top + innerHeight/2} 
                  textAnchor="middle" className="text-sm fill-gray-700 font-medium"
            ...
            {[0, 0.25, 0.5, 0.75, 1].map(t => (
              <g key={`x-${t}`}>
                <line x1={xScale(t)} y1={margin.top + innerHeight} 
                      x2={xScale(t)} y2={margin.top + innerHeight + 5} 
                      stroke="#94a3b8" strokeWidth="1" />
                <text x={xScale(t)} y={margin.top + innerHeight + 18} 
            ...
            {[0, 0.5, 1, 1.5].filter(t => t <= maxVelocity).map(t => (
              <g key={`y-${t}`}>
                <line x1={margin.left - 5} y1={yScale(t)} 
                      x2={margin.left} y2={yScale(t)} 
                      stroke="#94a3b8" strokeWidth="1" />
                <text x={margin.left - 10} y={yScale(t) + 4} 
            ...
            {result.positions.map(p => (
              <g key={p.acquirerId}
                 ...
                 onClick={() => setSelectedAcquirer(p === selectedAcquirer ? null : p)}
                 className="cursor-pointer">
                <circle cx={xScale(p.sectorBreadth)} cy={yScale(p.velocity)}
                        r={dotSize(p.dealCount)}
                        fill={QUADRANT_COLORS[p.classification]}
                        stroke={selectedAcquirer?.acquirerId === p.acquirerId ? '#000' : '#fff'}
                        strokeWidth={selectedAcquirer?.acquirerId === p.acquirerId ? 3 : 2}
                        opacity={hoveredAcquirer && hoveredAcquirer.acquirerId !== p.acquirerId ? 0.4 : 1}
                        className="transition-all" />
                <text x={xScale(p.sectorBreadth)} y={yScale(p.velocity) - dotSize(p.dealCount) - 5}
                      textAnchor="middle" className="text-xs fill-gray-700 font-medium pointer-events-none">
                  {p.acquirerName}
                </text>
              </g>
            ))}
          ...
          {hoveredAcquirer && (
            <div className="absolute top-2 right-2 bg-white border border-gray-200 rounded-lg p-3 shadow-lg max-w-xs">
              <div className="font-medium text-sm" style={{ fontFamily: "'Bodoni MT', Didot, serif" }}>
              ...
              <div className="text-xs text-gray-500 mt-2 space-y-0.5">
                ...
                <div>Breadth: {(hoveredAcquirer.sectorBreadth * 100).toFixed(0)}%</div>
              </div>
            </div>
          )}
        </div>
      ...
      {selectedAcquirer && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          <div className="flex items-start justify-between mb-3">
            <div>
              <h5 className="font-medium text-lg" style={{ fontFamily: "'Bodoni MT', Didot, serif" }}>
                {selectedAcquirer.acquirerName}
              </h5>
              <p className="text-sm text-gray-600">{selectedAcquirer.qualitativeDescription}</p>
            </div>
            <span className="text-xs px-2 py-1 rounded uppercase"
                  style={{ 
                    backgroundColor: QUADRANT_COLORS[selectedAcquirer.classification] + '20',
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        {result.patterns.length > 0 ? (
          ...
          <p className="text-sm text-gray-500 italic">No strong patterns detected in current sample</p>
        )}
      </div>
      ...
      <div className="bg-gradient-to-r from-[#E8B4B8] via-[#B8A9C9] to-[#4A5D8A] p-6 rounded-lg text-white">
        ...
        <p className="text-sm leading-relaxed">
          Mapped {result.positions.length} acquirers across breadth × velocity dimensions. 
          {result.patterns.length > 0 ? ' Identified ' + result.patterns.length + ' qualitative patterns.' : ' Patterns are weak.'} 
          <strong> This is exploratory pattern recognition, not statistical hypothesis testing.</strong> 
          Findings should be confirmed with larger dataset and qualitative validation (interviews with M&A teams).
        </p>
      </div>
    </motion.div>
  );
