...
export interface FounderClassification {
  ...
  nameOrigin?: 'western' | 'east_asian' | 'south_asian' | 'middle_eastern' | 'african' | 'other';
}
...
export default function GenderInferenceQuality({ 
  ...
  const stats = {
    ...
    mediumConfidence: founders.filter(f => f.confidence >= 0.65 && f.confidence < 0.85).length,
  ...
  const sourceBreakdown = {
    ...
    self_disclosed: founders.filter(f => f.source === 'self_disclosed').length,
    inferred: founders.filter(f => f.source === 'inferred').length
  };

  const expectedMisclassifications = Math.round(stats.total * (1 - overallAccuracy));
  ...
  const getGenderColor = (gender: string): string => {
    switch (gender) {
      ...
      case 'ambiguous': return 'bg-yellow-100 text-yellow-700';
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="bg-white border border-gray-200 p-4 rounded-lg text-center">
          <div className="text-3xl font-light" style={{ fontFamily: "'Bodoni MT', Didot, serif", color: '#22c55e' }}>
        ...
        <div className="bg-white border border-gray-200 p-4 rounded-lg text-center">
          <div className="text-3xl font-light" style={{ fontFamily: "'Bodoni MT', Didot, serif", color: '#e76f51' }}>
      ...
      <div className="bg-amber-50 border-l-4 border-amber-500 p-4 rounded-r-lg">
        <p className="text-sm text-amber-800">
          <strong>Critical Note:</strong> Out of {stats.total} founder classifications, 
          we expect ~{expectedMisclassifications} to be incorrect based on the {(overallAccuracy * 100).toFixed(0)}% provider accuracy. 
          This is BEFORE accounting for ambiguous names ({stats.ambiguous} flagged). 
          <strong> All downstream analyses must incorporate this measurement error.</strong>
        </p>
      </div>
        ...
        <div className="space-y-3">
          ...
          <div>
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className="text-gray-500">{stats.female} ({((stats.female / stats.total) * 100).toFixed(0)}%)</span>
            </div>
          ...
          <div>
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className="text-gray-500">{stats.male} ({((stats.male / stats.total) * 100).toFixed(0)}%)</span>
            </div>
            <div className="h-6 bg-gray-100 rounded-full overflow-hidden">
              <div 
          ...
          <div>
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className="text-gray-500">{stats.ambiguous} ({((stats.ambiguous / stats.total) * 100).toFixed(0)}%)</span>
            </div>
            <div className="h-6 bg-gray-100 rounded-full overflow-hidden">
              <div 
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-green-50 p-4 rounded-lg border border-green-200 text-center">
            <div className="text-2xl font-light" style={{ fontFamily: "'Bodoni MT', Didot, serif", color: '#22c55e' }}>
              {stats.highConfidence}
            </div>
            <div className="text-xs text-green-700 uppercase mt-1" style={{ fontFamily: "'Arial Narrow', sans-serif" }}>
              High Confidence
            </div>
          ...
          <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200 text-center">
            <div className="text-2xl font-light" style={{ fontFamily: "'Bodoni MT', Didot, serif", color: '#eab308' }}>
              {stats.mediumConfidence}
            </div>
            <div className="text-xs text-yellow-700 uppercase mt-1" style={{ fontFamily: "'Arial Narrow', sans-serif" }}>
              Medium Confidence
            </div>
          ...
          <div className="bg-red-50 p-4 rounded-lg border border-red-200 text-center">
            <div className="text-2xl font-light" style={{ fontFamily: "'Bodoni MT', Didot, serif", color: '#ef4444' }}>
              {stats.lowConfidence}
            </div>
            <div className="text-xs text-red-700 uppercase mt-1" style={{ fontFamily: "'Arial Narrow', sans-serif" }}>
              Low Confidence
            </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="max-h-64 overflow-y-auto">
          <table className="w-full text-xs">
            <thead className="text-gray-500 uppercase border-b border-gray-200" style={{ fontFamily: "'Arial Narrow', sans-serif" }}>
              <tr>
            ...
            <tbody>
              {filteredFounders.map((f, i) => (
                <tr key={i} className="border-b border-gray-100">
                  ...
                  <td className="py-2">
                    <span className={`px-2 py-0.5 rounded ${getGenderColor(f.inferredGender)}`}>
                      {f.inferredGender}
                    </span>
                  </td>
                  <td className="py-2">
                    <div className="flex items-center gap-2">
                      <div className="w-16 h-2 bg-gray-200 rounded-full overflow-hidden">
                        <div 
                          className={`h-full ${getConfidenceColor(f.confidence)}`}
                  ...
                  <td className="py-2 text-gray-500">{f.source.replace('_', ' ')}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          >
            <div className="bg-white p-4 rounded border border-gray-200">
              <p className="text-sm text-gray-600 mb-4">
                Gender inference accuracy varies by name origin. Non-Western names have higher 
                ambiguity rates and lower accuracy:
              </p>
              <table className="w-full text-sm">
                ...
                <tbody>
                  {Object.entries(INFERENCE_QUALITY_BY_ORIGIN).map(([origin, data]) => (
                    <tr key={origin} className="border-b border-gray-100">
                      <td className="py-2 font-medium capitalize">{origin.replace('_', ' ')}</td>
                      <td className="py-2 text-right">
                        <span className={data.accuracy >= 0.9 ? 'text-green-600' : data.accuracy >= 0.85 ? 'text-yellow-600' : 'text-red-600'}>
                          {(data.accuracy * 100).toFixed(0)}%
                        </span>
                      </td>
                      <td className="py-2 text-right text-gray-600">{(data.ambiguous * 100).toFixed(0)}%</td>
                      <td className="py-2 text-right text-gray-500 text-xs">n={data.sampleSize.toLocaleString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              <p className="text-xs text-gray-500 mt-3">
                <strong>Implication:</strong> If our dataset has many non-Western names, 
                actual error rate may exceed the {((1 - overallAccuracy) * 100).toFixed(0)}% baseline.
              </p>
            </div>
          </motion.div>
        )}
      ...
      <div className="bg-gray-900 text-white p-4 rounded-lg text-sm">
        ...
        <ul className="space-y-1 text-gray-300">
          <li>• Provider: {apiProvider} with {(overallAccuracy * 100).toFixed(0)}% reported accuracy</li>
