...
export async function GET(request: Request) {
  ...
  const bucket = rateLimit({ key: `genomics-variants:${ip}`, limit: 120, windowMs: 60_000 });
  if (!bucket.ok) {
  ...
  try {
    ...
    const { limit, offset } = parsePageParams(url.searchParams, { defaultLimit: 100, maxLimit: 1000 });
    ...
    if (!callsetId && !gene) {
      return NextResponse.json(
        { error: 'Provide callsetId or gene to avoid full-table scans on large catalogs' },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json({ error: 'Failed to query variants' }, { status: 500 });
  }
}
