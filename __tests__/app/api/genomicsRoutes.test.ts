...
describe('genomics API', () => {
  ...
  it('callsets returns 503 when variant store disabled (edge)', async () => {
    const { requireVariantStore } = await import('@/lib/genomics/variantStoreGuard');
    ...
    const response = await GET(new Request('http://localhost/api/genomics/callsets'));
  ...
  it('variants requires callsetId or gene (error)', async () => {
    ...
    const response = await GET(new Request('http://localhost/api/genomics/variants'));
