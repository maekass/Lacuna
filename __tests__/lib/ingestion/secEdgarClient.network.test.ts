...
describe('secEdgarClient (mocked fetch)', () => {
  ...
  it('loadSecTickerMap resolves tickers from SEC JSON (success)', async () => {
    ...
    const { loadSecTickerMap, resolveTicker } = await import('@/lib/ingestion/secEdgarClient');
  ...
  it('fetchRecentFilings filters by keyword and sinceDate (success)', async () => {
    ...
    const { fetchRecentFilings } = await import('@/lib/ingestion/secEdgarClient');
    const hits = await fetchRecentFilings(1477449, { sinceDate: '2020-06-15', limit: 10 });
  ...
  it('fetchRecentFilings throws on HTTP error (error)', async () => {
    ...
    const { fetchRecentFilings } = await import('@/lib/ingestion/secEdgarClient');
    await expect(fetchRecentFilings(1477449)).rejects.toThrow(/submissions failed/);
  });
  ...
  it('scanAcquisitionFilings skips unknown tickers (edge)', async () => {
    ...
    const { scanAcquisitionFilings } = await import('@/lib/ingestion/secEdgarClient');
    const hits = await scanAcquisitionFilings(['UNKNOWN'], { limitPerTicker: 1 });
    expect(hits).toEqual([]);
  });
});
