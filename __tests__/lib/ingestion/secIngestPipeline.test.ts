...
vi.mock('@/lib/ingestion/secEdgarConnector', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@/lib/ingestion/secEdgarConnector')>();
  return {
    ...actual,
...
vi.mock('@/lib/ingestion/monitoringAlerts', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@/lib/ingestion/monitoringAlerts')>();
  return {
    ...actual,
...
function sampleParsed(overrides: Partial<ParsedAcquisition> = {}): ParsedAcquisition {
  return {
...
describe('runSecIngest', () => {
  ...
  it('syncs classified deals when DATABASE_URL is set (success)', async () => {
    ...
    expect(mockLogIngestComplete).toHaveBeenCalledWith(
      expect.objectContaining({ scanned: expect.any(Number), parsed: 1, inserted: 1 }),
    );
  });
  ...
  it('throws when no tickers are available (error)', async () => {
    ...
    await expect(
      runSecIngest({
        dryRun: true,
        datasetPath: join(process.cwd(), '__tests__/helpers/empty-acquirers.fixture.json'),
      }),
