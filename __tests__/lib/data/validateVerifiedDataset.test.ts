...
describe('validateVerifiedDataset', () => {
  ...
  it('errors on invalid announcedDate format (error)', () => {
    ...
    expect(report.errors.some((e) => e.code === 'deal.announcedDate')).toBe(true);
  });
  ...
  it('errors on invalid provenance lastUpdated (error)', () => {
    ...
    expect(report.errors.some((e) => e.code === 'provenance.lastUpdated')).toBe(true);
  });
  ...
  it('errors when provenance disclaimer is empty (error)', () => {
    ...
    expect(report.errors.some((e) => e.code === 'provenance.disclaimer')).toBe(true);
  });
  ...
  it('warns when deal has no value and no note (warning)', () => {
    ...
    expect(report.warnings.some((w) => w.code === 'deal.undisclosedNote')).toBe(true);
  });
  ...
  it('warns on valuation without valuationSource (warning)', () => {
    ...
    expect(report.warnings.some((w) => w.code === 'company.valuationSource')).toBe(true);
  });
  ...
  it('warns on sector with companies but no deals (warning)', () => {
    ...
    expect(report.warnings.some((w) => w.code === 'stats.sectorNoDeals')).toBe(true);
  });
});
