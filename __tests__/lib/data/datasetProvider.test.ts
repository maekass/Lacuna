...
describe('datasetProvider', () => {
  ...
  it('getVerifiedDataset returns static JSON in static mode (success)', async () => {
    ...
    expect(dataset.provenance.disclaimer.toLowerCase()).toContain('educational');
  });
  ...
  it('getVerifiedDataset loads from DB in db mode (success)', async () => {
    ...
    vi.mocked(loadVerifiedDatasetFromDb).mockResolvedValue(minimalVerifiedDataset);
  ...
  it('getVerifiedDataset propagates DB loader errors (error)', async () => {
    vi.stubEnv('LACUNA_DATA_MODE', 'db');
    const { loadVerifiedDatasetFromDb } = await import('@/lib/data/loadVerifiedDatasetFromDb');
    vi.mocked(loadVerifiedDatasetFromDb).mockRejectedValue(new Error('connection refused'));
