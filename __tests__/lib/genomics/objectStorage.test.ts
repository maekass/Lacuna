...
describe('objectStorage', () => {
  ...
  it('builds local file URI by default (success)', () => {
    ...
    expect(buildObjectUri('demo/sample.vcf.gz')).toContain('data/variants/demo/sample.vcf.gz');
  });
  ...
  it('builds s3 URI when configured (success)', () => {
    ...
    expect(buildObjectUri('cohort-a/sample.vcf.gz')).toBe('s3://lacuna-variants/cohort-a/sample.vcf.gz');
  });
