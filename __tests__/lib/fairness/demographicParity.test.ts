...
const sampleCompanies: CompanyWithFounders[] = [
  ...
  company({
    companyId: 'c2',
    companyName: 'Ro',
    founders: [{ founder: 'B. Example', inferredGender: 'male', confidence: 0.92, source: 'common_name' }],
    wasAcquired: false,
  }),
  company({
    companyId: 'c3',
...
describe('calculateDemographicParity', () => {
  it('computes parity difference and CI (success)', () => {
    ...
    expect(result.confidenceInterval[0]).toBeLessThanOrEqual(result.parityDifference);
    expect(result.confidenceInterval[1]).toBeGreaterThanOrEqual(result.parityDifference);
  ...
  it('returns zero acquisition average when not acquired (edge)', () => {
    const result = analyzeFounderCharacteristics([
      ...
      company({
        companyId: 'c4',
        companyName: 'Peer Co',
        founders: [{ founder: 'C. Example', inferredGender: 'male', confidence: 0.9, source: 'common_name' }],
        wasAcquired: false,
      }),
    ]);
