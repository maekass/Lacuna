...
const companies: AcquiredCompany[] = minimalVerifiedDataset.companies.map((c) => ({
  id: c.id,
  name: c.name,
  sector: c.sector,
  stage: 'series_b',
...
const acquisitions: AcquisitionRecord[] = minimalVerifiedDataset.acquisitions.map((d) => ({
...
describe('analyzeVelocity', () => {
  it('builds yearly deal counts and trend (success)', () => {
    const velocity = analyzeVelocity(amazon, [
      { acquirerId: 'acquirer-amazon', companyId: 'c1', year: 2020, value: 100 },
      { acquirerId: 'acquirer-amazon', companyId: 'c2', year: 2021, value: 200 },
      { acquirerId: 'acquirer-amazon', companyId: 'c1', year: 2022, value: 150 },
    ]);
...
describe('compareAcquirerTypes', () => {
  it('groups acquisitions by acquirer type (success)', () => {
    const comparison = compareAcquirerTypes(
      [teladoc, amazon],
      [
        { acquirerId: 'acquirer-teladoc', companyId: 'c1', year: 2020, value: 100 },
        { acquirerId: 'acquirer-amazon', companyId: 'c2', year: 2021, value: 200 },
      ],
      companies,
    );
