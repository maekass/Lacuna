...
const VALID_TIERS: HealthEquityDataTier[] = ['cited_epidemiology', 'illustrative_static'];
...
describe('HEALTH_EQUITY_FOCUS_AREAS', () => {
  ...
  it('cited epidemiology rows link to EPIDEMIOLOGY_DATABASE conditions (success)', () => {
    const cited = HEALTH_EQUITY_FOCUS_AREAS.filter((a) => a.dataTier === 'cited_epidemiology');
    expect(cited.length).toBeGreaterThan(0);

    for (const area of cited) {
      expect(area.epidemiologyCondition).toBeTruthy();
      const match = EPIDEMIOLOGY_DATABASE.find((e) => e.condition === area.epidemiologyCondition);
      expect(match, `missing epidemiology row for ${area.epidemiologyCondition}`).toBeDefined();
  ...
  it('illustrative rows omit epidemiologyCondition (edge)', () => {
    const illustrative = HEALTH_EQUITY_FOCUS_AREAS.filter((a) => a.dataTier === 'illustrative_static');
  ...
  it('related sectors align with verified dataset sector vocabulary (edge)', () => {
    ...
    for (const area of HEALTH_EQUITY_FOCUS_AREAS) {
      for (const sector of area.relatedSectors) {
        expect(knownSectors.has(sector), `unexpected sector ${sector} on ${area.id}`).toBe(true);
      }
    }
  });
});
