...
Outcomes4Me, Nov 2022) as a **cited external baseline** crosswalked to the
verified M&A sample — not live patient data.
...
The report argues engagement metrics alone miss whether patients' preferences
and life goals influence diagnosis, treatment, and survivorship.
...
- **Generalizability:** Breast cancer care continuum mirrors many conditions,
  but figures are **not** automatically transferable to all women's-health M&A
  sectors
...
The UI leads with indices that match how reviewers read the report — not only
the unweighted mean (often ~47/100 because several “1 in 4” metrics sit at index
25).
...
Sector and keyword matches are **heuristic** (`heuristic_affinity` tier).
Curated links carry analyst notes and are preferred when tiers conflict.
...
- `portfolioCoveragePct` — all linked in sector / addressable (often 100% when
  sectors are broad)
- `curatedCoveragePct` — curated analyst links in sector / addressable
- `heuristicCoveragePct` — sector/keyword links only (excludes curated) /
  addressable
...
Analyst mappings in `patientEmpowermentCrosswalk.ts` may include public
citations:
...
**Evidence coverage %** = addressable companies with curated link **and**
`sourceUrl` / addressable sample. Heuristic % is the review queue; evidence % is
diligence-ready.
...
Checks metric completeness, curated link integrity (valid company/metric IDs),
and pipeline invariants on the static dataset.
