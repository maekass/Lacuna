...
GitHub Action [`.github/workflows/weekly-deal-pipeline.yml`](../.github/workflows/weekly-deal-pipeline.yml):
...
Vercel cron [`/api/cron/sec-ingest`](../src/app/api/cron/sec-ingest/route.ts) runs the same
hands-off chain when `LACUNA_HANDS_OFF_INGEST=true` (best with `LACUNA_DATA_MODE=db`).
...
Review queue still works at `/deals#data-pipelines` when you want to curate edge cases.
