...
`DRIVER_WEIGHTS` in `src/lib/quant/priors.ts`. Investigation only — scoring
behaviour was not changed for this document.
...
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------- |
| clinicalValidation  | `company.clinicalStage` at `predictionEngines.ts:39`; optional `company.clinicalEfficacy` at `:40-42`                                                                    | yes               | `clinicalStage` is proxied from the free-form `stage` string (`adaptQuantCompany.ts:34-45`, applied at `:101`). `proxyClinicalStage` maps any stage containing `"acquired"` to `fda_approved` (`:36`), the maximum bucket (score 10 at `predictionEngines.ts:37`). That is post-announcement information. `clinicalEfficacy` is never populated on the live path (`adaptQuantCompany.ts:106-107`), so those cut points (`africanRepresentation > 0.2`, `populationDiversity > 0.7`) do not fire in production — but the stage proxy does. | contaminated — reads post-outcome data             |
...
`ExitPredictor.tsx` `buildPredictions` (`:64-207`) is a separate heuristic. It
compares `company.valuation` to the median of acquired-company valuations
(`:80-85`, `:126-127`) and adds `+0.1` to `confidence` when the company is
already an acquisition target (`:183`). Those reads are post-outcome when the
row is an acquired company. That surface is outside the
`predictionEngines.ts:96-108` request but it is the other live score.
...
leakage-free. Treat every weight and every live cut point as **unknown**, except
`clinicalValidation`'s stage proxy, which is **contaminated** on any row whose
`stage` string contains `"acquired"`.
