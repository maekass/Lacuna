...
3. **Bound fan-out** — SEC classification uses `SEC_CLASSIFY_CONCURRENCY` (default 3). Evidence enrichment caps at 20 companies per click and prioritizes genomics/diagnostics targets first.
...
Lacuna does **not** host variant call sets or BAM/FASTA files. Genomics scalability here means large **catalogs of diagnostics companies, trials, and deal metadata**:
...
Multi-GB VCF blobs: object storage + ClickHouse variant catalog — see [GENOMICS_VARIANT_STORE.md](./GENOMICS_VARIANT_STORE.md). Disabled by default on Vercel (`LACUNA_VARIANT_STORE=off`).
