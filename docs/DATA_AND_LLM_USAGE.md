...
This document describes how Lacuna uses data sources and LLM (Large Language
Model) integrations for its analytics systems.
...
MCP provides structured data access for AI systems without direct LLM
integration.
...
**Note**: All future LLM uses would be for presentation/summarization only,
never for core analytical calculations.
