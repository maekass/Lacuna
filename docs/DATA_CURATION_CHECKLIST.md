...
Use this before merging any row into `src/data/dataset.verified.json`. Staging candidates live in `staging/deals_candidates.csv` (copy from `staging/deals_candidates.template.csv`). **Never auto-merge** from SEC scans or CSV imports.
...
Corporate buyers that are also portfolio companies (e.g. Ro) may appear as `companies.id` on `acquisitions.acquirerId`. Document in `dealValueNote` or provenance `notes[]`.
...
**Dual-attestation:** primary + secondary URL before `status=approved` in staging CSV.
