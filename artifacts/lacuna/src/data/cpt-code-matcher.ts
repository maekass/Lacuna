...
export class CPTCodeMatcher {
  ...
  async fuzzyMatch(
    ...
    let searchTerms = this.extractSearchTerms(
      productName,
      productDescription,
      sector,
    );
  ...
  private categorizeProcedure(description: string): string {
    ...
    if (
      desc.includes("ultrasound") || desc.includes("imaging") ||
      desc.includes("radiology")
    ) {
      return "Diagnostic Imaging";
    }
    if (
      desc.includes("surgery") || desc.includes("procedure") ||
      desc.includes("repair")
    ) {
      return "Surgical";
    }
    if (
      desc.includes("evaluation") || desc.includes("visit") ||
      desc.includes("consultation")
    ) {
      return "Evaluation & Management";
    }
    if (
      desc.includes("laboratory") || desc.includes("test") ||
      desc.includes("pathology")
    ) {
      return "Laboratory";
    }
    if (
      desc.includes("therapy") || desc.includes("treatment") ||
