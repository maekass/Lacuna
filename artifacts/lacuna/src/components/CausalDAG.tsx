...
const MEASURED_CONFOUNDERS = [
  {
    name: "Founding Year",
    proxy: "Company age at funding round",
    quality: "good",
  },
  {
    name: "Sector Momentum",
    proxy: "Total sector funding in year prior",
    quality: "moderate",
  },
  { name: "Valuation", proxy: "Last known valuation", quality: "good" },
  {
    name: "Network Size",
    proxy: "Number of prior investors",
    quality: "moderate",
  },
];
...
const UNMEASURED_CONFOUNDERS = [
  {
    ...
    concern:
      "Exceptional founders attract both better funding AND acquirers independently",
  ...
  },
  {
    ...
    concern:
      "Acquirer desperation drives both earlier acquisitions and willingness to pay premium",
  ...
  },
  {
    ...
    concern:
      "FDA approval pathways affect both company trajectory AND acquirer interest",
...
export default function CausalDAG() {
  ...
  const getNodeColor = (type: DAGNode["type"]) => {
    switch (type) {
      case "exposure":
        return "#4A5D8A"; // Cosmic Blue
      case "outcome":
        return "#5D4E6D"; // Deep Plum
      case "confounder":
        return "#B8A9C9"; // Lavender
      case "mediator":
        return "#E8B4B8"; // Transcendent Pink
      case "unmeasured":
        return "#e74c3c"; // Red for danger
      default:
        return "#95a5a6";
    }
  };
    ...
    >
      ...
      <div className="bg-white border border-lacuna-border rounded-lg p-6">
        ...
        >
          ...
          {DAG_EDGES.map((edge, index) => {
            ...
            if (
              !showUnmeasured &&
              (fromNode.type === "unmeasured" || toNode.type === "unmeasured")
            ) {
              return null;
            }
            ...
            <div className="bg-white p-4 rounded border border-lacuna-border space-y-4">
              <div>
                ...
                <ul className="text-sm space-y-1 text-lacuna-text-primary">
                  ...
                  <li>
                    • Sector Momentum → Funding Stage AND Sector Momentum →
                    Acquisition
                  </li>
                  <li>
                    • Network Size → Funding Stage AND Network Size →
                    Acquisition
                  </li>
                </ul>
              </div>
              ...
              <div className="bg-red-50 border-l-4 border-red-500 p-3 rounded-r">
                ...
                <p className="text-sm text-red-700">
                  • Founder Quality → Funding Stage AND Founder Quality →
                  Acquisition<br />
                  • Acquirer Urgency → Acquisition (affects timing
                  independently)<br />
                  • Regulatory Shifts → Sector Momentum AND Regulatory Shifts →
                  Acquisition
                </p>
                <p className="text-xs text-red-600 mt-2">
                  These paths remain open. We rely on sensitivity analysis to
      ...
      <div className="bg-red-50 border border-red-200 rounded-lg p-6">
        ...
        <div className="space-y-4">
          {UNMEASURED_CONFOUNDERS.map((c) => (
            <div
              key={c.name}
              className="bg-white p-4 rounded border border-red-100"
            >
              <h5
        ...
        <p className="text-sm text-red-700 mt-4 p-3 bg-white rounded border border-red-200">
          <strong>Sensitivity Analysis Required:</strong>{" "}
          We calculate Oster&apos;s δ to determine how strong these unmeasured
          confounders would need to be to flip our conclusions. See Sensitivity
          Analysis section for δ values.
        </p>
      </div>
      ...
      <div className="bg-gradient-to-r from-[#E8B4B8] via-[#B8A9C9] to-[#4A5D8A] p-6 rounded-lg text-white">
        ...
        <div className="space-y-2 text-sm">
          <p>
            <strong>We assume:</strong> Conditioning on{" "}
            {MEASURED_CONFOUNDERS.length}{" "}
            measured confounders blocks all backdoor paths, satisfying
            Pearl&apos;s backdoor criterion for measured variables.
          </p>
          <p>
            <strong>We acknowledge:</strong> {UNMEASURED_CONFOUNDERS.length}
            {" "}
            unmeasured confounders remain. Our causal claims are valid ONLY IF
            these are weak relative to measured confounders.
          </p>
          <p>
            <strong>We test:</strong>{" "}
            Sensitivity analysis (Oster&apos;s δ) quantifies how strong
            unmeasured confounding would need to be to invalidate our
            conclusions.
          </p>
          <p className="mt-3 pt-3 border-t border-white/30 font-medium">
            DO NOT interpret our results as causal without reviewing the
