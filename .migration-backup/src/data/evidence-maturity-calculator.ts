...
export function calculateEvidenceMaturity(
  ...
  const hasFlagshipPublication =
    input.trials.some((t) => t.isFlagshipJournal) ||
...
function calculateFDAScore(products: FDAProduct[]): number {
  ...
  const scores = products.map((p) =>
    FDA_CLEARANCE_SCORES[p.clearanceType] || 0
  );
...
function calculateClinicalResultsScore(trials: ClinicalTrial[]): number {
  ...
  const trialsWithResults =
    completedTrials.filter((t) => t.postedResults).length;
  const resultsRatio = trialsWithResults / completedTrials.length;

  return Math.round(resultsRatio * 100);
}

function hasResultsForPhase(
  trials: ClinicalTrial[],
  phase: TrialPhase,
): boolean {
...
function calculatePublicationScore(
  ...
  const trialRegularCount =
    trials.filter((t) => t.hasPublishedResults && !t.isFlagshipJournal).length;

  // Manual publications
  const manualFlagshipCount =
    manualPublications.filter((p) => p.isFlagship).length;
  const manualRegularCount =
    manualPublications.filter((p) => p.peerReviewed && !p.isFlagship).length;
...
function calculateAcquisitionContext(
  ...
  const activeTrials = trials.filter((t) => {
    ...
    const complete = t.completionDate
  ...
  if (hadFDAApproval) {
    const latestApproval = Math.max(
      ...approvalsBefore.map((p) => new Date(p.approvalDate).getTime()),
    );
    ...
    if (monthsSinceApproval < 0) {
      description = `Acquired ${
        Math.abs(monthsSinceApproval)
      } months BEFORE FDA approval`;
    } else if (monthsSinceApproval < 6) {
      description =
        `Acquired within 6 months of FDA approval (valuation trigger)`;
    } else {
