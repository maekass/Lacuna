...
describe("loadVerifiedDatasetFromDb", () => {
  ...
  it("loads and maps all tables (success)", async () => {
    mockQuery.mockImplementation((sql: string, params?: unknown[]) => {
      ...
      if (sql.includes("FROM companies")) {
        return Promise.resolve([sampleCompanyRow]);
      }
      if (sql.includes("FROM acquirers")) {
        return Promise.resolve([sampleAcquirerRow]);
      }
  ...
  it("throws when provenance row is missing (error)", async () => {
    ...
    await expect(loadVerifiedDatasetFromDb()).rejects.toThrow(
      "dataset_provenance row missing",
    );
  });
...
describe("loadAcquisitionsBySector", () => {
  ...
  it("queries with parameterized sector filter (success)", async () => {
    ...
    const { loadAcquisitionsBySector } = await import(
      "@/lib/data/loadVerifiedDatasetFromDb"
    );
