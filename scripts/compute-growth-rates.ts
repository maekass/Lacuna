...
interface Company { id: string; name: string; sector: string }
...
const secArtifact = existsSync(secPath)
  ...
  : { evidenceStatus: "withheld_missing_artifact", records: [] as SecRevenueRecord[] };

if (secArtifact.evidenceStatus?.startsWith("withheld")) {
  const output = {
    generatedAt,
    datasetHash,
    source: "Lacuna verified dataset + validated SEC annual revenue artifact",
    evidenceStatus: "withheld_pending_regeneration",
    reason: "SEC revenue evidence is currently withheld pending identity-validated regeneration. Financing-derived and valuation-derived rates are not substituted for operating growth.",
    companies: [],
    sectorMedians: {},
    method: "No company growth rate is published until the corrected SEC revenue artifact contains comparable annual operating revenue observations.",
    warning: "Missing operating data remains missing; financing and valuation changes are not substituted for revenue growth.",
  };
  writeFileSync("src/data/computed-growth-rates.json", JSON.stringify(output, null, 2) + "\n");
  console.log("✅ Operating growth withheld deterministically while SEC revenue evidence is unavailable.");
  process.exit(0);
}
...
const results: GrowthRateResult[] = companies.map((company) => {
  const rows = [...(byCompany.get(company.id) ?? [])].sort((a, b) => a.fiscalYear - b.fiscalYear);
  if (rows.length < 2) {
    return {
...
for (const sector of [...new Set(companies.map((company) => company.sector))]) {
  ...
  if (rates.length < 3) {
    sectorMedians[sector] = { medianCAGR: null, sampleSize: rates.length, confidence: rates.length ? "low" : "none" };
    continue;
  }
...
const output = {
  generatedAt,
  datasetHash,
  source: "Lacuna verified dataset + identity-validated SEC annual revenue artifact",
  ...
  warning: "Private-company operating growth is withheld when comparable sourced revenue observations are unavailable.",
};
writeFileSync("src/data/computed-growth-rates.json", JSON.stringify(output, null, 2) + "\n");
console.log(`✅ Revenue CAGR published for ${results.filter((row) => row.cagr !== null).length}/${results.length} companies.`);
