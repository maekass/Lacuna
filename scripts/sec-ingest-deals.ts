...
async function main() {
  if (!process.env.SEC_EDGAR_USER_AGENT?.trim()) {
    console.error('Set SEC_EDGAR_USER_AGENT before ingest (SEC fair-access policy).');
    console.error(
  ...
  console.log(`SEC ingest starting${dryRun ? ' (dry run — no DB writes)' : ''}…`);
  ...
  console.log(
    `Women's health candidates: ${result.classified.filter((c) => c.womensHealthRelevant).length}`,
  );

  const aiClassified = result.classified.filter((c) => c.classificationMethod === 'ai');
  const keywordClassified = result.classified.filter((c) => c.classificationMethod !== 'ai');
  if (aiClassified.length > 0) {
    const modelId = aiClassified[0]?.classificationModelId ?? 'unknown';
    console.log(`AI classification: ${aiClassified.length} filing(s) via ${modelId}`);
  }
