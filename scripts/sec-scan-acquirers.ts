...
import { formatHitsAsCsvRows, scanAcquisitionFilings } from '../src/lib/ingestion/secEdgarClient';
...
async function main() {
  if (!process.env.SEC_EDGAR_USER_AGENT?.trim()) {
    console.error('Set SEC_EDGAR_USER_AGENT before scanning (SEC fair-access policy).');
    console.error('Example: SEC_EDGAR_USER_AGENT="Lacuna Research you@example.com" npm run sec:scan');
    process.exit(1);
  }
  ...
  const hits = await scanAcquisitionFilings(tickers, {
    sinceDate: process.env.SEC_SCAN_SINCE ?? `${new Date().getFullYear() - 3}-01-01`,
  });
